...
def forward_pass(opts, transformer, iterations_per_step, is_training, outfeed, dense_queue, infeed):
    ...
    if not opts.pipeline:
        ...
        if is_training:
            ...
            # tf.train.get_or_create_global_step() returns a tf.Variable ...
            global_step_var = tf.train.get_or_create_global_step()

            # ... and tf.cast returns a tf.Tensor ...
            global_step_tensor = tf.cast(global_step_var, tf.int32)
            lr = make_lr_schedule(global_step_tensor)
            optimizer = make_optimizer(lr, last_itr)

            # ... and optimizer.minimize requires a tf.Variable,
            #     so we use the former here
            train_op = optimizer.minimize(metrics['training_loss'], global_step=global_step_var)
        else:
        ...
        with tf.control_dependencies([train_op]):
            with tf.variable_scope("counter", reuse=tf.AUTO_REUSE, use_resource=True):
                itr_counter = tf.get_variable("iterations", [], tf.int32, trainable=False)
            output = make_outfeed(lr, global_step_tensor, metrics, itr_counter)
        return output
    else:
    ...
    with tf.Session(graph=training_graph) as sess:
        ...
        for e in progress:
            ...
            for io_step in range(io_steps_per_epoch):
                ...
                # Dequeueing the outfeed returns a list if and only if the model is pipelined
                if not opts.pipeline:
