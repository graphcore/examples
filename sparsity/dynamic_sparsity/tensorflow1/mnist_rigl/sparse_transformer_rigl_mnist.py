...
def run_training(opts, transformer, x_train, y_train):
    ...
    logging.info(f"Batches per epoch: {batches_per_epoch} Train device iterations: {train_device_iterations}")
    ...
    with training_graph.as_default():
        with tf.device("cpu"):
            ...
            with scopes.ipu_scope("/device:IPU:0"):
                train_loop = partial(forward_pass, opts, transformer, lr_placeholder, train_device_iterations, True, train_outfeed, png_outfeed)
                train_loop = partial(loop_builder, train_device_iterations, train_loop, train_infeed)
    ...
    with tf.Session(graph=training_graph) as sess:
        ...
        for e in progress:
            for i in range(opts.steps_per_epoch):
                ...
                num_tokens = transformer.source_sequence_length * train_device_iterations * opts.batch_size
