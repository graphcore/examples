...
def make_bound_model(
        ...
        opt_cls,
        opt_kws,
        train_device_iterations,
        test_device_iterations,
        train_queues,
        test_queues,
        png_queue,
    ...
    def bound_train_model(inputs, labels):
        return model(
            ...
            opt_cls=opt_cls,
            opt_kws=opt_kws,
            iterations_per_dense_grad=train_device_iterations,
            training=True,
            disable_dense_grad=disable_dense_grad,
    ...
    def bound_train_loop():
        return loop_builder(
            iterations=train_device_iterations,
    ...
    def bound_test_model(inputs, labels):
        return model(
            ...
            opt_cls=opt_cls,
            opt_kws=opt_kws,
            iterations_per_dense_grad=test_device_iterations,
            training=False,
            disable_dense_grad=disable_dense_grad,
    ...
    def bound_test_loop():
        return loop_builder(
            iterations=test_device_iterations,
...
def make_bound_model_pipelining(
    ...
    def bound_train_loop(lr):
        ...
        return pipelining_ops.pipeline(
            ...
            repeat_count=train_device_iterations,
            inputs=[lr],
            device_mapping=[0, 0],
    ...
    def bound_test_loop():
        ...
        return pipelining_ops.pipeline(
            computational_stages=stages,
            gradient_accumulation_count=opts.gradient_accumulation_count,
            repeat_count=test_device_iterations,
...
def run_mnist(opts):
    ...
    test_device_iterations = num_test // (batch_size * opts.gradient_accumulation_count)

    train_device_iterations = opts.train_device_iterations
    if train_device_iterations is None:
        train_device_iterations = batches_per_epoch // opts.steps_per_epoch
    ...
    (bound_train_loop, bound_test_loop), train_inputs = bound_model_fn(
        ...
        opt_cls=opt_cls,
        opt_kws=opt_kws,
        train_device_iterations=train_device_iterations,
        test_device_iterations=test_device_iterations,
    ...
    logger.info(f"Epochs: {opts.epochs} Batch-size: {batch_size} Steps-per-epoch: {opts.steps_per_epoch} Train device iterations: {train_device_iterations}")
        ...
        if opts.test_mode in ["all", "training"]:
            ...
            for e in progress:
                for i in range(opts.steps_per_epoch):
                    ...
                    sess_time = t2 - t1
                    batch_time = sess_time / train_device_iterations
                    ...
                    batches_processed = train_device_iterations*steps
                    for name, fc in fc_layers.items():
                        if fc.is_sparse():
...
if __name__ == '__main__':
    ...
    parser.add_argument("--train-device-iterations", type=int, default=None,
                        help="Optional override for training device iterations to assist in profiling.")
