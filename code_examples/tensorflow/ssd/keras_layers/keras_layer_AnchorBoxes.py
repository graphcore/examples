...
class AnchorBoxes(Layer):
    ...
    def __init__(self,
        ...
        undefined. Some of these arguments are explained in more detail in the documentation of the `SSDBoxEncoder`
        class.
        ...
        Arguments:
            ...
            coords (str, optional): The box coordinate format to be used internally in the model (i.e. this is not the
                ...
                (box center coordinates, width, and height),'corners' for the format `(xmin, ymin, xmax,  ymax)`, or
                'minmax' for the format `(xmin, xmax, ymin, ymax)`.
            normalize_coords (bool, optional): Set to `True` if the model uses relative instead of absolute coordinates,
                i.e. if the model predicts box coordinates within [0,1] instead of absolute coordinates.
        """
        if K.backend() != 'tensorflow':
            raise TypeError("This layer only supports TensorFlow at the moment, but you are using the {} backend.".
                            format(K.backend()))
    ...
    def call(self, x, mask=None):
        ...
        # Compute the offsets, i.e. at what pixel values the first anchor box center point will be from the top
        # and from the left of the image.
        ...
        cy = np.linspace(offset_height * step_height, (offset_height + feature_map_height.value - 1) *
                         step_height, feature_map_height.value)
        cx = np.linspace(offset_width * step_width, (offset_width + feature_map_width.value - 1) * step_width,
                         feature_map_width.value)
        ...
        boxes_tensor = np.zeros((feature_map_height.value, feature_map_width.value, self.n_boxes, 4))
