...
def make_train_data(train_uv, train_values, args):
    ...
    print(f"Effective batch-size: {effective_batch_size} "
          ...
          f"Training steps-per-epoch: {steps_per_epoch} "
    ...
    def wait_until_finished(self, epoch):
        if self.eval_process:
            ...
            try:
                ...
                items = re.findall("PSNR:.*$", text, re.MULTILINE)
                try:
                    match = items[-1]
                    psnr = float(match.split()[1])
                    print(f"PSNR at epoch {epoch}: {psnr}")
                except:
                    print(f"Could not parse output:\n{text}")
                    psnr = None

                # Log the result if successful:
                if self.summary_writer and psnr is not None:
