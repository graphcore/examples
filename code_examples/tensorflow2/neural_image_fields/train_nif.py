...
def create_model(input_dim, input_dtype, layer_size, num_layers, use_siren, fp16_sin):
...
if __name__ == '__main__':
    ...
    strategy = su.get_train_strategy(args.no_ipu, args.replicas)

    with strategy.scope():
        model = create_model(train_uv.shape[-1], train_uv.dtype,
                             args.layer_size, args.layer_count, args.siren, args.sin_fp16)
        if not args.no_ipu:
        ...
        if args.tensorboard_dir is None:
            ...
            tb_logdir = args.tensorboard_dir
