...
class OutfeedOptimizer(Optimizer):
    ...
    def __init__(self, wrapped_optimizer, outfeed_queue,
        ...
        Args:
            ...
            outfeed_optimizer_mode: Used to select when gradients will be enqueued.
                This only changes the observed behaviour when using pipelining.
                Defaults to `OutfeedOptimizerMode.BEFORE_APPLY` which means that the
                selected gradients will be enqueued inside the `apply_gradients`
                function before it calls `apply_gradients` on the wrapped optimizer.
                When using gradient accumulation (for example, when pipelining) these
                will be the accumulated gradients. The
                alternative mode is `OutfeedOptimizerMode.AFTER_COMPUTE` which means
                that the selected gradients will be enqueued in the `compute_gradients`
                function after they are computed by the wrapped optimizer. When
                using gradient accumulation (for example, when pipelining) this
                allows the pre-accumulated gradients to be inspected.
    ...
    def compute_gradients(self, loss, var_list=None, **kwargs):
        ...
        "_grad" appended.
        If a filter was provided then it will be used to select which gradients to
        outfeed - the variable name must include one of the filter elements.
