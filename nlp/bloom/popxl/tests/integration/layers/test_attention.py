...
from modelling.decoder import BloomSelfAttentionTP2D


@torch.no_grad()
def test_attention(test_config: BloomConfig):
    ...
    # HF forward
    input_t = torch.rand((1, seq_len, hidden_size))
    ...
    z = hf_model.input_layernorm(input_t)
    output_, *_ = hf_model.self_attention(z, residual=input_t, attention_mask=mask_t, alibi=alibi_t)
    ...
    with ir.main_graph:
        ...
        ff_args, ff_graph = BloomSelfAttentionTP2D(test_config).create_graph(x)
    ...
    weights = BloomSelfAttentionTP2D.hf_mapping(test_config, ff_vars, hf_model)
