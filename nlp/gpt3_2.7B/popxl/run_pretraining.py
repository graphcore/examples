    ...
    with session:
        ...
        while True:
            ...
            for data in train_dl:
                data_map = {}
                words = to_numpy(
                    data["input_ids"][:, : config.model.sequence_length], session.inputs.words.dtype
                ).reshape(-1, *session.inputs.words.shape)
                labels = to_numpy(data["labels"], session.inputs.labels.dtype).reshape(-1, *session.inputs.labels.shape)
                lr = (
                    np.full(
                        (session.ir.num_host_transfers, config.execution.data_parallel * n_shards, 1), lr_schedule[step]
                    )
                    .astype("float32")
                    .squeeze()
                )
                ...
                data_map[session.inputs.words] = tensor_parallel_input(
                    ...
                    replicas,
                    partial(
                        GPTEmbeddingsTP.offset_input, vocab_size=config.model.embedding.vocab_size, n_shards=n_shards
                    ),
                )
...
def main():
    # Configuration
    config, args = gpt_pretraining_setup(CONFIG_DIR / "pretraining.yml", "release", "tiny")
