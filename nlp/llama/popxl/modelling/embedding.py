...
class LlamaEmbeddingsTP(addons.Module):
    ...
    def hf_mapping(config: LlamaConfig, variables: NamedTensors, hf_model: HFModel) -> Dict[popxl.Tensor, np.ndarray]:
        ...
        return {
            variables.word.weight: shard(
                pad(to_numpy(hf_model.embed_tokens.weight.data, dtype), word_pad), n_shards, axis=0
            ),
        }

    @staticmethod
    def get_offsets(config: LlamaConfig) -> np.ndarray:
        n_shards = config.execution.tensor_parallel
    ...
    @staticmethod
    def get_vocab_shard_sizes(config: LlamaConfig) -> int:
        n_shards = config.execution.tensor_parallel
    ...
    @classmethod
    def offset_inputs(cls, config: LlamaConfig, words: HostTensor, axis: int = 0):
        n_shards = config.execution.tensor_parallel
        word_offsets = LlamaEmbeddingsTP.get_offsets(config)

        words_offsetted = cls._offset_input(words, word_offsets, n_shards, axis)
        return words_offsetted

    @staticmethod
    def offset_input(data: np.ndarray, i: int, config: LlamaConfig):
        return data - (
