...
bool produced_by_dropout(popart::Tensor *t) {
  return t->hasProducer() &&
         t->getProducer()->isConvertibleTo<popart::DropoutBaseOp>();
}
...
public:
  bool matches(popart::Op *op) const override {
    if (op->isConvertibleTo<popart::AddOp>()) {
      return produced_by_dropout(
                 op->inTensor(popart::AddOp::getArg0InIndex())) &&
...
static popart::PatternCreator<ResidualAddInPlacePattern>
    c1("ResidualAddInPlacePattern", true);
