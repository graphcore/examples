...
def gpt_config_setup(
    ...
    config, args = parse_args_with_presets(GPTConfig, config_file, presets_key, default, custom_args, CLI_args)
    ...
    if hf_model_setup:
        if args.hf_model == "None":
            ...
            with timer("Loading HF model to host"):
                pretrained = HFGPT2Model.from_pretrained(args.hf_model)
...
def xl_hf_config_check(config: GPTConfig, hf_config: HFGPT2Config):
    ...
    params = [
        ...
        ("max_positional_length", config.model.embedding.vocab_size, hf_config.vocab_size),
    ]
