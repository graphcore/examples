...
def run_inference_hf(dataset, tokenizer, hf_model, sequence_length,
                     output_length, micro_batch_size):
    ...
    def next_token(inputs, lengths):
        ...
        return torch.argmax(next_token_logits,
                            dim=-1).reshape(-1)

    answers = batch_inference(unwrap(dataset),
                              next_token,
                              sequence_length,
...
def run_inference_popxl(config: GPTJConfig, dataset, tokenizer, hf_model,
...
def main():
    ...
    hf_model.eval()
    run_inference_hf(dataset, tokenizer, hf_model, max_len + output_length,
                     output_length, config.execution.micro_batch_size)

    # --- POPXL example ---
    run_inference_popxl(config, dataset, tokenizer, hf_model,
                        max_len + output_length, output_length)
