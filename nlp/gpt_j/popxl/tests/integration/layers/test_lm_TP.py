...
def test_lm_TP_cmp_huggingface(test_config: GPTJConfig):
    ...
    # HuggingFace
    config = HFConfig(n_layer=test_config.model.layers,
    ...
    # Offset inputs
    words_offsetted = GPTJEmbeddingsTP.offset_inputs(
...
def test_lm_to_hf(test_config: GPTJConfig):
    ...
    np.testing.assert_almost_equal(output_HF.reshape(
        fwd_data_full.shape), fwd_data_full, 3)
