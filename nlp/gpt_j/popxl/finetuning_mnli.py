...
from popxl_addons.rts import (
    all_gather_replica_sharded_graph, replica_sharded_spec, reduce_replica_sharded_graph)
...
def create_embeddings_graph(config: GPTJConfig, optimizer: addons.Module, *args, **kwargs):
    ...
    fwd_facts, embeddings.fwd = GPTJEmbeddingsTP(
...
def create_decoder_block_graph(config: GPTJConfig, optimizer: addons.Module, *args, **kwargs):
    ...
    # Create Graphs for computing forward, gradient and optimizer
    fwd_facts, layer.fwd = GPTJDecoderBlockTP(
        config).create_graph(*args, **kwargs)
    ...
    if config.execution.attention_serialisation > 1:
        ...
        assert len(
            layer.fwd.graph.called_graphs) == 1, "expected exactly 1 called graph by decoder layer fwd"
        blk_graph = GraphWithNamedArgs(layer.fwd.graph.called_graphs[0])
        grad_blk_graph = addons.transforms.autodiff(
            blk_graph, grads_required=blk_graph.graph.inputs[:-2])
...
def embeddings_batch_serialise(config: GPTJConfig, embeddings: Graphs, input_streams: addons.InputStreams,
...
def decoder_block_batch_serialise(config: GPTJConfig, layer: Graphs, x_buffer: popxl.RemoteBuffer,
...
def head_batch_serialise(config: GPTJConfig, head_graph: GraphWithNamedArgs, input_streams: addons.InputStreams,
    ...
    with timer('PopXL IR construction'):
        ...
        with main:
            ...
            for key in ('fwd', 'bwd', 'optim'):
                ...
                if key in embeddings.facts.keys():
                    transformer.insert(f"embeddings.{key}",
                                       embeddings.facts[key].init_remote(
                                           embeddings.buffers[key], 0, f"embeddings.{key}", empty=empty),
                                       overwrite=True)
                ...
                if key in head.facts.keys():
                    variables.insert(f"lm_head.{key}",
                                     head.facts[key].init_remote(
                                         head.buffers[key], 0, "lm_head", empty=empty),
                                     overwrite=True)
            ...
            with popxl.in_sequence():
                ...
                def embedding_fwd_phase(seed):
                    ...
                    embeddings_vars = embeddings.fwd_all_gather(
                        embeddings_vars)
                    # Forward
                ...
                def layer_optim(n: popxl.Tensor, lr: popxl.Tensor, grad_norm: popxl.Tensor):
                    ...
                    optimizer_step(decoder_block.optim,
                                   layer_vars, lr, grad_norm)
                    decoder_block.optim_fwd_store(layer_vars, n)
                    return n + 1

                i = popxl.constant(0, name="layer_index")
                layer_optim_graph = ir.create_graph(
                    layer_optim, i, lr, grad_norm)
                if config.execution.code_load:
                ...
                def head_optim(lr: popxl.Tensor, grad_norm: popxl.Tensor):
                    ...
                    task_head_optimizer_step(
                        head.optim, head_vars, lr, grad_norm)
                    # Store
        ...
        fwd_vars = NamedTensors.from_dict({
            ...
            'transformer.decoder': NamedTensors.from_dict({
                i: variables.transformer.decoder[i].fwd
...
def main():
    ...
    config, *_ = gptj_config_setup(CONFIG_DIR / "finetuning_mnli.yml", "release",
                                   "gptj_6B_1024_pod64", wandb_setup=False, hf_model_setup=False)
