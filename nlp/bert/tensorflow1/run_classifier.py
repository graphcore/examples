    ...
    with train_graph.as_default():
        ...
        if opts["do_training"] and opts["current_mode"] == "train":
            input_file = os.path.join(
                opts["output_dir"],
                f"train_{opts['task_name']}_{opts['task_type']}.tf_record",
            )
        elif opts["do_evaluation"] and opts["current_mode"] == "eval":
            input_file = os.path.join(
                opts["output_dir"],
                f"eval_{opts['task_name']}_{opts['task_type']}.tf_record",
            )
        elif opts["do_predict"] and opts["current_mode"] == "predict":
            input_file = os.path.join(
                opts["output_dir"],
                f"predict_{opts['task_name']}_{opts['task_type']}.tf_record",
            )
        else:
...
def training_step(train, learning_rate, i, opts):
    ...
    if opts["task_type"] == "regression":
        ...
        # no output if using synthetic data
        if opts["synthetic_data"] and len(pred) == 0:
            return np.nan, np.nan, batch_time, np.nan, np.nan
        else:
        ...
        if opts["synthetic_data"] and len(logits) == 0:
            return np.nan, batch_time, np.nan, np.nan
        else:
            loss = np.mean(_loss)
            acc = np.mean(acc)
...
def predict_step(predict):
    ...
    if opts["task_type"] == "regression":
        ...
        loss = np.mean(loss)
        acc = np.mean(acc)
        tmp_output = {"loss": loss, "acc": acc, "preds": preds}
...
def main(opts):
    ...
    tokenizer = tokenization.FullTokenizer(
        vocab_file=os.path.expandvars(opts["vocab_file"]), do_lower_case=opts["do_lower_case"]
    )
    ...
    if opts["do_training"]:
        ...
        if opts["steps_per_ckpts"]:
            ...
            ckpt_iterations = opts['device_iterations'] * \
        ...
        if opts["task_type"] == "regression":
            ...
            print_format = (
                "step: {step:6d}, iteration: {iteration:6d} ({percent_done:.3f}%),  epoch: {epoch:6.2f}, lr: {lr:6.4g}, loss: {loss:6.3f}, accuracy: {acc:6.3f} %, "
                "throughput: {throughput_samples_per_sec:6.2f} samples/sec, batch time: {avg_batch_time:8.6f} s, total_time: {total_time:8.1f} s")
        step = 0
        start_all = time.time()
        i = 0
            ...
            if i % log_iterations == 0:
                ...
                if opts["task_type"] == "regression":
                    train_summary.value.add(
                        tag="pearson", simple_value=pearson
                    )
                    train_summary.value.add(
                        tag="spearman", simple_value=spearman
                    )
                else:
                ...
                train_summary.value.add(
                    tag="througput", simple_value=throughput
                )
    ...
    if opts["do_evaluation"]:
        ...
        iterations = int(
            len(eval_examples) // (opts["micro_batch_size"] * opts["gradient_accumulation_count"]) + 1
        )
        ...
        while i < iterations:
            try:
                ...
                all_time_consumption.append(
                    duration / opts["device_iterations"])
            except tf.errors.OpError as e:
                raise tf.errors.ResourceExhaustedError(
                    e.node_def, e.op, e.message
                )
        ...
        if opts["task_type"] == "regression":
            ...
            if opts["wandb"]:
                wandb.log(
                    {
                        "average_pearson": tmp_output["average_pearson"],
    ...
    if opts["do_predict"]:
        ...
        tf.logging.info(
            "  Predict batch size = %d", opts["predict_batch_size"]
        )
        ...
        if (
            opts["init_checkpoint"] and not opts["do_training"] and opts["do_predict"]
        ):
            finetuned_checkpoint_path = opts["init_checkpoint"]
        else:
        ...
        if finetuned_checkpoint_path:
            ...
            saver_restore.restore(
                prediction.session, finetuned_checkpoint_path
            )
            print("Done.")
        ...
        iterations = int(
            len(predict_examples) // (opts["micro_batch_size"] * opts["gradient_accumulation_count"]) + 1
        )
...
def set_training_defaults(opts):
    ...
    if "glu_reg" in [
        layer for stage in opts["pipeline_stages"] for layer in stage
    ]:
...
if __name__ == "__main__":
    ...
    if opts["wandb"]:
        ...
        wandb.init(
            project="tf-bert", sync_tensorboard=True, name=opts["wandb_name"]
        )
