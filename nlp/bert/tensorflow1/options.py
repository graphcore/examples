...
def add_common_arguments(parser, required=True):
    ...
    group.add_argument('--ipu-replica-identical-seed', default=False, type=bool,
                       help="Whether to seed all the local replica IPUs identically.")
    ...
    group.add_argument('--device-iterations', type=int, default=1,
                       help="Number of iterations to perform on the device before returning to the host.")
    group.add_argument('--available-memory-proportion', type=float, nargs="+", default=0.23,
    ...
    group.add_argument('--sloppy-parallel-interleave', type=bool, default=True, help="Whether tfrecords should be read without determinism. A lack of determinism improves throughput at the expense of reproducibility. For multi-instance runs, this is set to False for training correctness.")
...
def create_command_line_parser():
    parser = argparse.ArgumentParser(
        description="""BERT on Graphcore's IPU""",
        add_help=False,
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    group = parser.add_argument_group("""Main options""")
    group.add_argument(
        "--help", action="store_true", default=False, help="Display help."
    )
    group.add_argument(
        "--task", type=str, choices=["pretraining"], help="Type of NLP task."
    )
    group.add_argument(
        "--config", type=str, help="BERT configuration file in JSON format."
    )
    return parser
...
def make_global_options(task_specific_parsers=[]):
    ...
    unknown_options = [
        opt
        for opt in opts_from_config_file.keys()
