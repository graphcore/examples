...
class AxProcessor(MnliProcessor):
...
class MnliMismatchProcessor(MnliProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            if i == 0:
                continue
            guid = "%s-%s" % (set_type, i)
            text_a = tokenization.convert_to_unicode(line[text_index])
            if set_type == "test":
                label = "0"
            else:
                label = tokenization.convert_to_unicode(line[1])
...
class QqpProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            guid = "%s-%s" % (set_type, tokenization.convert_to_unicode(line[0]))
            try:
                text_a = tokenization.convert_to_unicode(line[q1_index])
                text_b = tokenization.convert_to_unicode(line[q2_index])
                label = 0 if test_mode else tokenization.convert_to_unicode(line[5])
            except IndexError:
                continue
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label))
...
class SyntheticProcessor(DataProcessor):

    def get_train_examples(self, path_placeholder):
        """See base class."""
        examples = []
        for k in range(10000):
            example = InputExample(guid=k,
                                   text_a="some text sequence a",
                                   text_b="some text sequence b",
                                   label="0")
            examples.append(example)
        return examples

    def get_dev_examples(self, path_placeholder):
        """See base class."""
        examples = []
        for k in range(10000):
            example = InputExample(guid=k,
    ...
    def get_test_examples(self, path_placeholder):
        """See base class."""
        examples = []
        for k in range(10000):
            example = InputExample(guid=k,
...
class SyntheticProcessorRegression(DataProcessor):
...
def convert_single_example(ex_index, example, label_list, seq_length,
                           tokenizer):
...
def file_based_convert_examples_to_features(
        examples, label_list, seq_length, tokenizer, output_file):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if isinstance(feature.label_id, float):
            features["label_ids"] = create_float_feature([feature.label_id])
        else:
            features["label_ids"] = create_int_feature([feature.label_id])
        features["num_labels"] = create_int_feature([feature.num_label])
...
def file_based_input_fn_builder(opts, is_training):
    input_file = opts['input_file']
    seq_length = opts['seq_length']
    drop_remainder = opts['drop_remainder']
...
def get_glue_dataset(opts, is_training):
    """Load the GLUE datasets, for training, eval, or predict."""
