...
def synthetic_pretraining_dataset(opts):
    ...
    if opts["static_mask"]:
        _input_ids = np.random.randint(
            vocab_size, size=(generated_dataset_size, seq_length)
        ).astype(np.int32)
        _input_position = np.random.randint(
            seq_length, size=(generated_dataset_size, seq_length)
        ).astype(np.int32)
        _token_type_ids = np.random.randint(
            4, size=(generated_dataset_size, seq_length)
        ).astype(np.int32)
        _masked_lm_positions = np.random.randint(
            seq_length, size=(generated_dataset_size, max_predictions_per_seq)
        ).astype(np.int32)
        _masked_lm_ids = np.random.randint(
            vocab_size, size=(generated_dataset_size, max_predictions_per_seq)
        ).astype(np.int32)
        _masked_lm_weights = np.random.randint(
            2, size=(generated_dataset_size, max_predictions_per_seq)
        ).astype(np.float16)
        _next_sentence_labels = np.random.randint(
            1000, size=(generated_dataset_size, 1)
        ).astype(np.int32)
        _mask_padding_index = np.random.randint(
            20, size=(generated_dataset_size, 1)
        ).astype(np.int32)
        _seq_padding_index = np.random.randint(
            128, size=(generated_dataset_size, 1)
        ).astype(np.int32)
        name_dict = {
        ...
        _input_mask = np.random.randint(
            2, size=(generated_dataset_size, seq_length)
        ).astype(np.int32)
        _segment_ids = np.random.randint(
            2, size=(generated_dataset_size, seq_length)
        ).astype(np.int32)
        _masked_lm_positions = np.random.randint(
            seq_length, size=(generated_dataset_size, max_predictions_per_seq)
        ...
        _next_sentence_labels = np.random.randint(
            2, size=(generated_dataset_size, 1)
        ).astype(np.int32)
        name_dict = {
            "input_ids": _input_ids,
...
def get_pretraining_dataset(
    opts, data_type, is_training=True, num_cpu_threads=4, use_static_mask=False
):
    if is_training:
    ...
    if use_static_mask:
        ...
        name_to_features = {
            ...
            "input_position": tf.FixedLenFeature([max_seq_length], tf.int64),
            "segment_ids": tf.FixedLenFeature([max_seq_length], tf.int64),
            "mask_padding_index": tf.FixedLenFeature([1], tf.int64),
            "seq_padding_index": tf.FixedLenFeature([1], tf.int64),
            "masked_labels": tf.FixedLenFeature(
    ...
    if is_training:
        ...
        if opts["distributed_worker_count"] > 1:
            # if num_instances > 1, sloppy=False. o/w each instance will read the tfrecord files in a different
            # order, and so will shard a different dataset. This would make it possible for several replicas to
            # process the same sample during a weight update step and so can affect convergence
            d = d.apply(
                parallel_interleave(
            ...
            # even more randomness to the training pipeline. If deterministic
            # runs are required, sloppy should be set to False
            d = d.apply(
                parallel_interleave(
                    tf.data.TFRecordDataset,
                    sloppy=opts["sloppy_parallel_interleave"],
                    cycle_length=cycle_length,
                )
            )
            d = d.shuffle(buffer_size=1000, seed=opts["seed"])
