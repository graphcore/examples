...
# and data/wikipedia/AA/sequence_512/wiki_XX_tokenised
...
# the config file. For example: bert/configs/pretrain_base_128.json
...
#    --mask-tokens
# must be provided with the same values written in
# the config file. For example: bert/configs/pretrain_base_512.json
...
mkdir -p data/wikipedia/AA/sequence_512
for i in $(seq -f "%02g" 0 6); do (nohup python bert_data/create_pretraining_data.py --input-file data/wikipedia/preprocessed/wiki_${i}_cleaned --output-file data/wikipedia/AA/sequence_512/wiki_${i}_tokenised --vocab-file data/ckpts/uncased_L-12_H-768_A-12/vocab.txt --sequence-length 512 --mask-tokens 76 --duplication-factor 6 --do-lower-case >> /dev/null &); done
# Wait until the first 7 tokenisation processes finish
ps aux | grep sequence_512 | grep -v grep | awk '{print $2}' | while read pid; do tail --pid=${pid} -f /dev/null; done
for i in $(seq -f "%02g" 7 13); do (nohup python bert_data/create_pretraining_data.py --input-file data/wikipedia/preprocessed/wiki_${i}_cleaned --output-file data/wikipedia/AA/sequence_512/wiki_${i}_tokenised --vocab-file data/ckpts/uncased_L-12_H-768_A-12/vocab.txt --sequence-length 512 --mask-tokens 76 --duplication-factor 6 --do-lower-case >> /dev/null &); done
# Wait until the other 7 tokenisation processes finish
ps aux | grep sequence_512 | grep -v grep | awk '{print $2}' | while read pid; do tail --pid=${pid} -f /dev/null; done
