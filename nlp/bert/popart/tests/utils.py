...
def run_py(proto: onnx.ModelProto,
    ...
    data_flow = popart.DataFlow(
        device_iterations, {output: popart.AnchorReturnType("ALL")
    ...
    # Add a device_iterations dimension if needed
    if device_iterations > 1:
        data = {k: np.repeat(v[np.newaxis], device_iterations, 0)
                for k, v in data.items()}
