...
def test_iteration_stats(task, epochs, seed, exponent_loss, exponent_acc):
    ...
    device_iterations = 500
    recording_steps = 1  # --aggregate-metrics-over-steps

    dataset_length = int(total_data_length / device_iterations)  # len(dataset)
    if total_data_length % device_iterations != 0:
        raise ValueError(
            "Dataset not divisible by bps, not supported in this test.")

    args = MockArgs(**{"epochs": epochs, "task": task, "device_iterations": device_iterations})
