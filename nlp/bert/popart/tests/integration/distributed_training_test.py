...
class TestBuildAndRun(SubProcessChecker):
    ...
    def _get_bert_command(self, extra_args=None):
        base_cmd = ("poprun --num-replicas 2 --ipus-per-replica 2 --num-instances 2"
                    " python bert.py --config configs/pretrain_large_128.json "
                    " --global-batch-size 32 --micro-batch-size 1 --gradient-accumulation-factor 16 --replication-factor 1 --generated-data --epochs 1"
                    " --replicated-tensor-sharding false --encoder-start-ipu 1 --layers-per-ipu 1 --num-layers 1 --device-iterations 1 --no-model-save --steps-per-log 1 "
