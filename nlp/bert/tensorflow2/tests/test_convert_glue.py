...
class TestConvertHFToFunctionalBertClassification:
    ...
    def setup_class(cls):
        ...
        samples, raw_dataset = load_dataset('mrpc',
        ...
        with cls.strategy.scope():
            ...
            cls.orig_model = TFBertForSequenceClassification.from_pretrained(
                'bert-base-uncased')
            cls.orig_outputs = cls.orig_model(cls.sample_inputs)

            set_random_seeds(seed=42)
            ipu_subclass_model = TFBertForSequenceClassification.from_pretrained(
                'bert-base-uncased')
            ...
            qa_loss = wrap_loss_in_enqueuer(ClassificationLossFunction,
                                            ["labels"])()
    ...
    def test_inference_outputs(self):
        np.testing.assert_almost_equal(
            self.orig_outputs['logits'].numpy(),
            self.sub_outputs['logits'].numpy(),
            decimal=5
        )
...
@pytest.mark.parametrize('task_name,task',
                         [("cola", ("sentence",)),
