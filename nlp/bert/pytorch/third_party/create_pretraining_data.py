...
"""
This script has been adapted from the original google-research/bert repo found here:
...
def write_instance_to_example_files(
    instances, tokenizer, max_seq_length, mask_tokens, output_files, ignore_index_value=0, max_samples=-1
):
...
def create_training_instances(
    input_files, tokenizer, max_seq_length, dupe_factor, short_seq_prob, mlm_prob, mask_tokens, rng
):
...
def create_instances_from_document(
    all_documents, document_index, max_seq_length, short_seq_prob, mlm_prob, mask_tokens, vocab_words, rng
):
...
def create_masked_lm_predictions(tokens, mlm_prob, mask_tokens, vocab_words, rng, max_seq_length):
    ...
    num_to_predict = min(mask_tokens, max(1, int(round(len(tokens) * mlm_prob))))
...
def main(args):
    ...
    write_instance_to_example_files(
        ...
        args.mask_tokens,
        output_files,
        args.ignore_index_value,
        args.max_samples,
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser("PreTraining Data for Mlm/Next Sentence prediction")
