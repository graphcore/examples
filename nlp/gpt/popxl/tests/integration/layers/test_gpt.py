...
from modelling.gpt_lm import GPTModel


def test_gpt_cmp_huggingface(test_config: GPTConfig):
    ...
    with main:
        ...
        args, graph = GPTModel(test_config).create_graph(input_ids=input_t)
        ...
        words_grad_d2h = addons.host_store(
            grad_map[graph.args.embeddings.word.weight])
        pos_grad_d2h = addons.host_store(
            grad_map[graph.args.embeddings.positional.weight])

    # Map weights from huggingface
    weights = GPTModel.hf_mapping(test_config, vars, hf_model)
