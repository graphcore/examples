...
class GPTEmbeddings(addons.Module):
    ...
    def build(self,
              ...
              position_ids: Optional[popxl.Tensor] = None,
              seed: Optional[popxl.Tensor] = None) -> popxl.Tensor:
        if position_ids is None:
            position_ids = popxl.constant(generate_positions(self.config),
                                          popxl.uint32,
                                          name="positions")
    ...
    def hf_mapping(config: GPTConfig, variables: NamedTensors,
        ...
        word_shard_size, pos_shard_size = GPTEmbeddings.get_vocab_shard_sizes(config)
        ...
        return {
            variables.word.weight: pad(to_numpy(hf_model.wte.weight.data, dtype), word_pad),
            variables.positional.weight: pad(to_numpy(hf_model.wpe.weight.data, dtype), pos_pad)
        }
