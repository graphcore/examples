...
def reshape_for_scores(x: popxl.Tensor, sequence_length: int,
                       heads: int) -> popxl.Tensor:
    assert len(x.shape) == 2
    micro_batch_size = x.shape[0] // sequence_length
    head_hidden_size = x.shape[1] // heads
    return x.reshape(
        (micro_batch_size, sequence_length, heads, head_hidden_size))
...
class GPTAttentionHeads(addons.Module):
    ...
    def build(self, x: popxl.Tensor, seed: Optional[popxl.Tensor] = None):
        ...
        def transform_heads(x: popxl.Tensor, is_key: bool) -> popxl.Tensor:
            return transpose_for_scores(
                reshape_for_scores(x, self.config.model.sequence_length,
                                   self.n_heads), is_key)
        ...
        query_length, key_length = attn_weights.shape[-2], attn_weights.shape[
            -1]
        # TODO: This causes a large rearrangement. Check tile mapping of broadcast add
        causal_mask = popxl.constant(np.tril(
            np.ones((query_length, key_length))),
            attn_weights.dtype,
            name="causal_mask")
        ...
        if not self.config.model.eval:
            ...
            attn_scores = ops.dropout(attn_scores,
                                      seed,
                                      p=self.config.model.dropout_prob)
    ...
    @staticmethod
    def hf_mapping(config: GPTConfig, vars: NamedTensors,
                   hf_model: GPT2Attention) -> Dict[popxl.Tensor, np.ndarray]:
        dtype = config.model.dtype
        hf_query_w, hf_key_w, hf_value_w = np.split(
            hf_model.c_attn.weight.data.numpy(), 3, axis=-1)
        hf_query_b, hf_key_b, hf_value_b = np.split(
            hf_model.c_attn.bias.data.numpy(), 3, axis=-1)
        ...
        return {
            ...
            vars.qkv.bias:
            np.concatenate((query_b, key_b, value_b), axis=-1),
        }


class GPTSelfAttention(addons.Module):
    def __init__(self, config: GPTConfig):
        super().__init__()
        self.config = config
        self.ln_1 = LayerNorm()
        self.heads = GPTAttentionHeads(config)
    ...
    def build(self, x: popxl.Tensor,
        ...
        # in original hf impl layer norm and final add are in the decoder block,
        # but we are incorporating them in this layer for better model partitioning
        # z = self.heads(x, heads_seed)
        # z = self.output(z)
        z = self.ln_1(x)
    ...
    def hf_mapping(config: GPTConfig, variables: NamedTensors,
                   hf_model: GPT2Block) -> Dict[popxl.Tensor, np.ndarray]:
        dtype = config.model.dtype
        return {
            variables.ln_1.weight:
            to_numpy(hf_model.ln_1.weight.data, dtype),
            variables.ln_1.bias:
            to_numpy(hf_model.ln_1.bias.data, dtype),
            variables.output.weight:
            np.ascontiguousarray(
                to_numpy(hf_model.attn.c_proj.weight.data, dtype)),
            variables.output.bias:
            to_numpy(hf_model.attn.c_proj.bias, dtype),
            **GPTAttentionHeads.hf_mapping(config, variables.heads, hf_model.attn)
        }
...
class GPTSelfAttentionTP(addons.Module):
    def __init__(self, config: GPTConfig):
        ...
        self.heads = GPTAttentionHeads(config=config,
    ...
    @staticmethod
    def hf_mapping(config, variables: NamedTensors,
                   hf_model: GPT2Block) -> Dict[popxl.Tensor, np.ndarray]:
        dtype = config.model.dtype
        n_shards = config.execution.tensor_parallel

        hf_query_w, hf_key_w, hf_value_w = np.split(
            hf_model.attn.c_attn.weight.data.numpy(), 3, axis=-1)
        hf_query_b, hf_key_b, hf_value_b = np.split(
            hf_model.attn.c_attn.bias.data.numpy(), 3, axis=-1)
        ...
        return {
            ...
            variables.ln_1.bias:
            to_numpy(hf_model.ln_1.bias.data, dtype),
            variables.heads.qkv.weight:
            np.ascontiguousarray(
                np.concatenate([
                    np.concatenate([query_w[i], key_w[i], value_w[i]],
                                   axis=-1)[np.newaxis, ...]
                    for i in range(n_shards)
                ])),
            variables.heads.qkv.bias:
            np.ascontiguousarray(
                np.concatenate([
                    np.concatenate([query_b[i], key_b[i], value_b[i]],
                                   axis=-1)[np.newaxis, ...]
                    for i in range(n_shards)
                ])),
            variables.output.weight:
            shard(c_proj_w, n_shards, axis=0),
            variables.bias:
            c_proj_b,  # Copied
        }
