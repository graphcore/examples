...
from popxl_addons import VariableFactory, NamedTensors
...
class GPTFeedForward(addons.Module):
    ...
    def hf_mapping(config: GPTConfig, variables: NamedTensors, hf_model: HFModel) -> Dict[popxl.Tensor, np.ndarray]:
        ...
        return {
            ...
            # HF GPT2MLP
            variables.intermediate.weight: np.ascontiguousarray(to_numpy(hf_model.mlp.c_fc.weight.data, dtype)),
            variables.intermediate.bias: to_numpy(hf_model.mlp.c_fc.bias, dtype),
            variables.output.weight: np.ascontiguousarray(to_numpy(hf_model.mlp.c_proj.weight.data, dtype)),
            variables.output.bias: to_numpy(hf_model.mlp.c_proj.bias, dtype),
        }
