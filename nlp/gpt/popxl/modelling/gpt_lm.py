...
from popxl_addons.layers import Linear, Embedding
...
from popxl_addons.named_replica_grouping import NamedReplicaGrouping, fill_none_group
...
class GPTLMHeadModel(addons.Module):
    def __init__(self, config: GPTConfig):
        """GPT model (GPT2-GPT3 architecture) with a language model head"""
        super().__init__()
        self.config = config
        self.transformer = GPTModel(config, include_layer_norm=False)  # move layer norm to the head
    ...
    def hf_mapping(config: GPTConfig, variables: NamedTensors, hf_model: HFModel) -> Dict[popxl.Tensor, np.ndarray]:
        dtype = config.model.dtype
        weights = GPTModel.hf_mapping(config, variables.transformer, hf_model.transformer, layer_norm=False)
        weights.update(GPTLMHead.hf_mapping(config, variables.lm_head, hf_model.transformer))
        return weights
...
class GPTLMHeadLossAndGrad(addons.Module):
    def __init__(self, config: GPTConfig):
        super().__init__()
        self.config = config

    def build(self, x: popxl.Tensor, labels: popxl.Tensor, word_embedding_t: popxl.Tensor,
              word_embedding_accum_t: popxl.Tensor):
        word_embed = popxl.TensorSpec((self.config.model.hidden_size, self.config.model.embedding.vocab_size),
                                      dtype=x.dtype)

        fwd_facts, fwd_graph = GPTLMHead(self.config).create_graph(x, word_embed)
        ...
        loss, dx = self.loss(logits, labels)
    ...
    def loss(self, logits: popxl.Tensor, labels: popxl.Tensor):
        vocab_size = self.config.model.embedding.vocab_size
        logits = logits.reshape_(
            (self.config.execution.micro_batch_size, self.config.model.sequence_length, vocab_size))
        loss, dx = addons.ops.cross_entropy_with_grad(logits,
                                                      labels,
                                                      loss_scaling=self.config.execution.loss_scaling,
                                                      ignore_index=-100)
        dx = dx.reshape_((-1, vocab_size))
        return loss, dx
...
class GPTLMHeadLossAndGradTP(addons.Module):
    ...
    def build(self, x: popxl.Tensor, labels: popxl.Tensor, word_embedding_t: popxl.Tensor,
              word_embedding_accum_t: popxl.Tensor, word_offset: popxl.Tensor):
