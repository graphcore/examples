...
class Yolov4_loss(nn.Module):
    ...
    def compute_object_loss(self, flattened_prediction, indices, mask, tbox, head_idx, anchor_ind):
        ...
        return self.focal_loss(prediction_objectness.float(), true_objectness[:flattened_prediction.shape[0]]).mean() * self.balance[head_idx]
    ...
    def focal_loss(self, pred, target):
        loss = torch.nn.functional.binary_cross_entropy_with_logits(pred, target, reduction='none', pos_weight=self.pos_weight)
        if self.fl_gamma > 0:
