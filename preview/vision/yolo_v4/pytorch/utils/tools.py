...
class StatRecorder:
    ...
    def logging(self, output_function, run_coco_eval):
        ...
        output_function(
            "Average Latency per Batch: {:.3f} ms".format(1000 * sum(avg_latency) / len(self.inference_times))
        )
        output_function(
            "throughput: {:.3f} samples/sec".format(sum(self.inference_throughputs) / len(self.inference_throughputs))
        )
        output_function("--------------------------------------------------")
