...
from .base_dataset import TextVideoDataset
...
class MSRVTT(TextVideoDataset):
    def _load_metadata(self):
        ...
        split_dir = os.path.join(
            self.metadata_dir, 'high-quality', 'structured-symlinks')
        ...
        train_df = pd.read_csv(os.path.join(
            split_dir, train_list_path), names=['videoid'])
        test_df = pd.read_csv(os.path.join(
            split_dir, test_list_path), names=['videoid'])
        self.split_sizes = {'train': len(train_df), 'val': len(
            test_df), 'test': len(test_df)}
        ...
        if js_test_cap_idx_path is not None and self.split != 'training':
            caps = pd.Series(np.load(os.path.join(
                split_dir, js_test_cap_idx_path), allow_pickle=True))
    ...
    def _get_caption(self, sample):
        ...
        if self.split in ['training', 'inference'] and caption_sample == "rand":
