...
class TextVideoDataset(Dataset):
    ...
    def _fix_temporal_samples(self):
        ...
        self.metadata['fix_start'] = self.metadata['frame_intervals'].apply(
            lambda x: np.arange(
                0, int(x[-1] / len(x - 1)), self.sliding_window_stride)
        )
    ...
    def __getitem__(self, item):
        ...
        try:
            ...
            if video_loading == 'strict':
                ...
                imgs = Image.new(
                    'RGB', (self.video_params['input_res'], self.video_params['input_res']), (0, 0, 0))
        ...
        meta_arr = {'raw_captions': caption,
                    'paths': rel_fp, 'dataset': self.dataset_name}
...
def read_frames_av(video_path, num_frames, sample='rand', fix_start=None):
    ...
    try:
        frames = []
        frames = [torch.from_numpy(f.to_rgb().to_ndarray())
