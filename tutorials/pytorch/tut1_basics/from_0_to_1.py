...
# From 0 to 1 - introduction to PopTorch

# This tutorial covers the basics of model making in PyTorch, using `torch.nn.
# Module`, and the specific methods to convert a PyTorch model to a PopTorch
# model so that it can be run on a Graphcore IPU.
...
# wrap your existing PyTorch model in either a PopTorch inference wrapper or a
# PopTorch training wrapper. You can provide further annotations to partition
# the model across multiple IPUs.

# You can wrap individual layers in an IPU helper to designate which IPU they
# should go on. Using the user-provided annotations, PopTorch will use [PopART]
# (https://docs.graphcore.ai/projects/popart-user-guide/en/latest/intro.html)
# to parallelise the model over the given number of IPUs. Additional
# parallelism can be expressed via a replication factor which enables you to
# data-parallelise the model over more IPUs.

# Under the hood PopTorch uses [TorchScript](https://pytorch.org/docs/stable/jit.html),
# an intermediate representation (IR) of a PyTorch model, using the `torch.jit.# trace` API. That means it inherits the constraints of that API. These include:
...
# To learn more about TorchScript and JIT, you can go through this [tutorial]
# (https://pytorch.org/tutorials/beginner/Intro_to_TorchScript_tutorial.html).

# PopTorch has been designed to require fewer manual alterations to your models
...
# implemented by the backend yet and you can find the list of supported
# operations [here](https://docs.graphcore.ai/projects/poptorch-user-guide/en/latest supported_ops.html).
...
# ### Import the packages
# PopTorch is a separate package from PyTorch, and available in Graphcore's
# Poplar SDK. Both must thus be imported:
import torch
import poptorch
import torchvision
import numpy as np
...
# Under the hood, PopTorch uses Graphcore's high-performance deep learning
# framework PopART. It is therefore necessary to enable in your environment
# PopART as well as Poplar.
...
# tensors. However this is unlikely to be the most efficient way and can result
# in data loading being a bottleneck to the model, slowing down the training
# process. In order to make data loading easier and more efficient, there's the
...
# simply use PyTorch's API, including `torch.nn.Module`. The difference with
# what we're used to with pure PyTorch is the _loss computation_, which has to
# be part of the `forward` function. It is to ensure the loss is computed on
...
# and updating the weights are the process we refer to as the "training loop".
...
# run on the device independently to the host. To achieve that virtual
...
# evaluating your model on CPU is slow because the test dataset is large and/or
...
# PyTorch practitioners to use IPUs.
...
# is one cycle of that loop, which run entirely on the IPU (the device), and
...
# ### `randomSeed`
# The IPU has a different, on-device pseudo-random number generator (PNRG).
# This option sets the seed for the PNRG on the IPU. This is equivalent to
# using `torch.seed`.
...
# In the meantime, to learn more about the IPU and the lower level library
# Poplar, you can go through our Poplar tutorials and read our Poplar SDK
# overview.
