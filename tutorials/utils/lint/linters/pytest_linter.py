...
from ...tutorials_tests.modern_python_tests import test_must_use_pytest
...
def apply_lint_function(changed_files: List[Path]) -> int:
    ...
    try:
        ...
        with contextlib.redirect_stdout(f), contextlib.redirect_stderr(f):
            test_must_use_pytest.test_must_use_pytest(changed_files)
        return 0
    except AssertionError as err:
        print("test_must_use_pytest failed with:")
        print(str(err))
        return 1
