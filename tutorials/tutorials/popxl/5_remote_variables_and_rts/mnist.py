...
def remote_step(
    ...
    buffers = addons.named_variable_buffers(facts, entries=1, shard_over_dict=shard_over)
        ...
        with popxl.in_sequence(True):
            ...
            if opts.data_parallel > 1:
                ...
                grad_reduce, names = reduce_replica_sharded_graph(
                    grads,
                    "mean",
                    shard_groups=NamedReplicaGrouping.from_dict(get_shard_groups(opts, bwd_facts)),
            ...
            optimizer_step(sharded_vars, grads_dict, optimizer, counter, opts, shard_groups)
            ...
            if opts.data_parallel > 1:
                for name, v in sharded_vars.named_tensors.items():
                    if v.meta_shape:
                        ...
                        ops.var_updates.copy_var_update_(variables.named_tensors[name], v_full)
...
def main():
    ...
    parser.add_argument("--data-parallel", type=int, default=4, help="data parallelism (default: 4)")
    parser.add_argument(
