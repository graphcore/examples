...
def load_data(batch_size):
    ...
    if train_data_len % batch_size:
        print(
            f"WARNING: the size of the training dataset ({train_data_len})"
    ...
    if test_data_len % batch_size:
        print(
            f"WARNING: the size of the test dataset ({test_data_len})"
            f" is not divisible by the chosen batch size ({batch_size})."
...
def parse_params():
    ...
    if args.gradient_accumulation_steps_per_replica and not args.pipelining:
        print(
            "Note: `--gradient-accumulation-steps-per-replica` controls the number of"
