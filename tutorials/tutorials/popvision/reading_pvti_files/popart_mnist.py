...
def train(opts):
    ...
    train_data, train_labels, test_data, test_labels = (
        load_dummy(opts) if opts.syn_data_type in ["random_normal", "zeros"] else load_mnist()
    )
    ...
    with get_device(opts.num_ipus, opts.simulation) as device:
        ...
        cpu_load_graph = pvti.Graph("CPULoad", "%")
        cpu_series = cpu_load_graph.addSeries("CPU")
        ...
        for i in range(opts.epochs):
            ...
            with pvti.Tracepoint(mnistPvtiChannel, f"Epoch:{i}"):
                ...
                with pvti.Tracepoint(mnistPvtiChannel, "Training"):
                    if i > 0:
                    ...
                    for step, (data, labels) in enumerate(training_set):
                        ...
                        # Record the CPU Load
                        cpu_series.add(psutil.cpu_percent(interval=None))
                        ...
                        results = np.argmax(
                            training.anchors[output].reshape([training_set.inputs_per_step, 10]),
                            1,
                        )
                        num_correct = np.sum(results == labels.reshape([training_set.inputs_per_step]))
                        training_accuracy_series.add(num_correct / training_set.inputs_per_step)
                ...
                if not opts.validation_final_epoch or i == opts.epochs - 1:
                    ...
                    with pvti.Tracepoint(mnistPvtiChannel, "Evaluation"):
                        ...
                        for step, (data, labels) in enumerate(test_set):
                            ...
                            validation_accuracy_series.add(num_correct / test_set.inputs_per_step)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="Set the Batch size." " This must be a multiple of the replication factor.",
    )
    parser.add_argument(
        "--batches-per-step",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Output extra performance information, specify with" " either 'training' or 'inference'",
    )
    parser.add_argument(
        "--syn-data-type",
    ...
    if opts.pipeline and opts.num_ipus != 2 * opts.replication_factor:
        raise Exception(
            "Invalid Argument: For this model, pipelining is only " "valid if each replica contains two IPUs."
        )
