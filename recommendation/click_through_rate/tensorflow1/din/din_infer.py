        ...
        with ipu_scope('/device:IPU:0'):
            def comp_fn_validate():
                def body(uids, mids, cats, mid_his, cat_his, mid_mask, target, seqlen):
                    ...
                    return outfeed_op
                return loops.repeat(opts['device_iterations'], body, [], infeed_val)
...
def inference(opts):
    ...
    iterations = VALIDATION_DATA_SIZE * opts['epochs'] / (opts['micro_batch_size'] * opts['replicas'])
    ...
    with uid_embedding.register(infer_graph.session), mid_embedding.register(infer_graph.session), cat_embedding.register(infer_graph.session):
        while i < iterations:
            ...
            i += opts['device_iterations']
            accuracy = np.mean(acc)
            accs.append(accuracy)
            prob_1 = prob.reshape([opts['device_iterations']*opts['micro_batch_size'], 2])
            prob_1 = prob_1[:, 0].tolist()
            target_1 = target.reshape([opts['device_iterations']*opts['micro_batch_size'], 2])
...
def add_dataset_arguments(parser):
    ...
    group.add_argument('--device-iterations', type=int, default=160, help='Number of iterations/ global batches to perform on the device before returning to the host')
    return parser
