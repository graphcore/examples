    ...
    with graph.as_default():
        ...
        with ipu_scope('/device:IPU:0'):
            def comp_fn():
                ...
                return loops.repeat(opts['device_iterations'], body, [tf.constant(0, data_type)]*3, infeed)
...
def train_process(opts, restore=False):
    ...
    steps = epochs * iterations_per_epoch // opts["device_iterations"]
    ...
    with uid_embedding.register(graph.session), mid_embedding.register(graph.session), cat_embedding.register(graph.session):
        while i < steps:
            ...
            batch_throughput = opts["micro_batch_size"] * opts["device_iterations"] / time_one_iteration
    ...
    if steps > 1:
        throughput = steps * opts["device_iterations"] * opts['micro_batch_size'] / total_time
