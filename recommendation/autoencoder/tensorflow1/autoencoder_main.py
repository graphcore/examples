    ...
    with train_graph.as_default():
        ...
        with ipu_scope('/device:IPU:0'):
            ...
            def comp_fn():
                ...
                return loops.repeat(opts.device_iterations,
                                    body,
        ...
        rmse = sum_rmse_metric / opts.device_iterations
        loss = total_loss / opts.device_iterations
    ...
    with valid_graph.as_default():
        ...
        with ipu_scope('/device:IPU:{}'.format(tf_device_ordinal)):
            def comp_fn():
                ...
                return loops.repeat(opts.validation_device_iterations,
                                    body,
                                    [tf.constant(0, tf.float32)],
                                    infeed)
        ...
        # Accuracy Ops
        rmse = sum_rmse_metric / opts.validation_device_iterations
...
def train_process(opts, training_data, valid_data):
    ...
    lrs = [base_lr * opts.micro_batch_size * decay for decay in decay_lr]
    epochs = opts.epochs
    iterations_per_epoch = training_data.size / \
        (opts.micro_batch_size * opts.device_iterations)
    ...
    for e in range(iterations):
        ...
        epoch = float(opts.device_iterations * opts.micro_batch_size *
        ...
        if not (e + 1) % opts.steps_per_log or e == 0:
            stats = {
                ...
                'items_per_sec': opts.device_iterations * opts.micro_batch_size / avg_batch_time}
            ...
            if opts.multiprocessing:
                ...
                print(
                    "Validation RMSE (step {}, epoch {:6.2f}, users/sec {:6.2f}): {:6.4f}".format(
                        ...
                        1,
                        epoch,
                        opts.validation_micro_batch_size *
...
def validation_process(opts, valid_data, q=None):
    ...
    opts.validation_device_iterations = valid_data.size // opts.validation_micro_batch_size
...
def preprocess_options(opts):
    ...
    if opts.training_data_file:
        ...
        micro_batch_size_dict = {'3m_train.txt': [64, 128],
                                 '6m_train.txt': [64, 256],
                                 '1y_train.txt': [32, 256],
        ...
        if not opts.micro_batch_size:
            if dataset_suffix in micro_batch_size_dict.keys():
                opts.micro_batch_size = micro_batch_size_dict[dataset_suffix][0]
            else:
                raise Exception("\n\nError: Unrecognised training dataset file name: \"{}\"\n"
                                "Either set micro-batch and graph sizes manually, or ensure "
                                "training dataset file name is one of the following:"
                                "\n\t{}, {}, {}, {}\n".format(dataset_suffix, *micro_batch_size_dict))
        if not opts.size:
            if dataset_suffix in micro_batch_size_dict.keys():
                opts.size = micro_batch_size_dict[dataset_suffix][1]
            else:
                raise Exception("\n\nError: Unrecognised training dataset file name: \"{}\"\n"
                                "Either set micro-batch and graph sizes manually, or ensure "
                                "training dataset file name is one of the following:"
        ...
        if not opts.epochs:
            opts.epochs = 2
        if not opts.validation_micro_batch_size:
            opts.validation_micro_batch_size = 128
        if not opts.micro_batch_size:
            opts.micro_batch_size = 64
        if not opts.size:
            opts.size = 128

    if not opts.validation_micro_batch_size:
        opts.validation_micro_batch_size = opts.micro_batch_size

    # Logs and checkpoint paths
    name = "bs{}-rn{}-{}".format(opts.micro_batch_size,
...
def get_options():
    ...
    group.add_argument(
        ...
        type=int,
        default=None,
        help='Number of samples processed together on-device in one forward and backward pass of the algorithm. '
             'It is automatically set depending on training dataset file name if not set manually '
    ...
    group.add_argument(
        ...
        type=int,
        default=None,
        help="Number of samples processed together on-device in one forward pass for validation graph. "
             "Set to (training) micro-batch-size if not specified")
    group.add_argument(
        '--precision',
        type=str,
        default="16.16",
    ...
    group.add_argument(
        '--validation-batches-per-step',
        type=int,
        default=250,
        help="Number of micro-batches per replica performed sequentially on device before "
             "returning to the host. Increase this number to improve accelerator utilization."
    )
    group.add_argument(
        '--steps-per-log',
        type=int,
    ...
    group.add_argument(
        ...
        type=int,
        default=250,
        help="Number of iterations/ global batches performed sequentially on device before "
             "returning to the host. Increase this number to improve accelerator utilization.")
    group.add_argument(
        '--logdir',
        type=str,
    ...
    group.add_argument(
        ...
        type=str,
        help="Only run testing on checkpoint directory specified (example: "
             "./log-dir/weights-bs64-rn128-20200331_160859/). "
             "Testing data path is taken from validation data file.")
...
if __name__ == '__main__':
    ...
    if not opts.no_validation:
        log_str += ("Validation Graph\n"
                    ...
                    " Micro-batch Size {validation_micro_batch_size}\n")
