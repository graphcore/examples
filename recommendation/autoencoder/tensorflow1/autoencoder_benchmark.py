...
class IPUGraphParams:
    def __init__(self,
        ...
        self.iterations_per_epoch = training_data.size / (opts.batch_size * opts.device_iterations)
...
def train_process(graphParams, opts, training_data, device_index, q=None):
    ...
    for e in range(graphParams.iterations):
        ...
        tot_users_iterated += opts.device_iterations * opts.batch_size
        graphParams.throughputs.append(opts.device_iterations * opts.batch_size / batch_time)
        ...
        if not (e + 1) % opts.steps_per_log or e == 0 or e == graphParams.iterations - 1:
            ...
            epoch = float(opts.device_iterations * opts.batch_size *
            ...
            stats = {
                ...
                'train_acc': train_acc,
                'it_time': batch_time,
                'latest_users_per_sec': int(opts.device_iterations * opts.batch_size / batch_time),
...
def get_options():
    ...
    group.add_argument(
        ...
        type=int,
        default=250,
        help="How many iterations/global batches to perform on the device before returning to the host.")
    group.add_argument(
        '--logdir',
        type=str,
        default="./logdir",
