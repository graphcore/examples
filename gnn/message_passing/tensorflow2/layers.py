...
flags.DEFINE_enum('gather_scatter', 'grouped', ('grouped', 'debug', 'dense'),
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        if FLAGS.decoder_mode == 'node_global':
            node_inputs_to_global_decoder = _scatter(nodes, node_graph_idx, num_segments=FLAGS.n_graphs_per_pack,
            ...
            inputs_to_decoder = _scatter(nodes, node_graph_idx, num_segments=FLAGS.n_graphs_per_pack,
                                         gather_scatter_method=FLAGS.gather_scatter)
...
class GraphNetworkLayer(tf.keras.layers.Layer):
    def __init__(self, edge_model_fn, node_model_fn, global_model_fn, use_residual=True,
        ...
        self.node_dropout = xpu.Dropout(rate=nodes_dropout, noise_shape=(FLAGS.micro_batch_size, FLAGS.n_nodes_per_pack, 1))
        self.edge_dropout = xpu.Dropout(rate=edges_dropout, noise_shape=(FLAGS.micro_batch_size, FLAGS.n_edges_per_pack, 1))
        self.global_dropout = xpu.Dropout(rate=globals_dropout,
                                          noise_shape=(FLAGS.micro_batch_size, FLAGS.n_graphs_per_pack, 1))
    ...
    def call(self, inputs, training=False):
        ...
        # ---------------NODE step---------------
        adjacent_edges = _scatter(edges_update, receivers, num_segments=FLAGS.n_nodes_per_pack,
                                  gather_scatter_method=FLAGS.gather_scatter)
        ...
        # ---------------GLOBAL step---------------
        node_aggregate = _scatter(nodes_update, node_graph_idx, num_segments=FLAGS.n_graphs_per_pack,
                                  gather_scatter_method=FLAGS.gather_scatter)
        edge_aggregate = _scatter(edges_update, edge_graph_idx, num_segments=FLAGS.n_graphs_per_pack,
                                  gather_scatter_method=FLAGS.gather_scatter)
...
class GinEncoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        nodes = self.atom_encoder(nodes)
        return nodes, edges, receivers, senders, tf.zeros(
            [FLAGS.micro_batch_size, FLAGS.n_graphs_per_pack, 1]), node_graph_idx, edge_graph_idx
...
class GinDecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        nodes, edges, receivers, senders, global_latent, node_graph_idx, edge_graph_idx = inputs
        graphwise_means = _batched_segment_mean(nodes, node_graph_idx,
                                                graphs_per_pack=FLAGS.n_graphs_per_pack,
                                                gather_scatter_method=FLAGS.gather_scatter)
...
class GraphIsomorphismLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        message = _scatter(sent_attributes, receivers, num_segments=FLAGS.n_nodes_per_pack,
                           gather_scatter_method=FLAGS.gather_scatter)
...
def _scatter(data, indices, num_segments, gather_scatter_method='grouped'):
    if gather_scatter_method == 'grouped':
        ...
        unbatched_xs = [tf.math.unsorted_segment_sum(data[i], indices[i], num_segments=num_segments)
...
def _batched_segment_mean(x, indices, graphs_per_pack, gather_scatter_method='debug'):
    ...
    if gather_scatter_method == 'grouped':
        ...
        n_nodes_per_graph = xpu.grouped_scatter(dummy_nodes[..., tf.newaxis], indices,
                                                table_size=graphs_per_pack)
        ...
        graphwise_means = [
            tf.math.unsorted_segment_mean(x[i], indices[i], num_segments=graphs_per_pack)
