...
def grouped_gather(params: tf.Tensor, indices: tf.Tensor) -> (tf.Tensor, Callable):
    ...
    def _internal_grouped_gather(_params, _indices):
        ...
        (output,) = ipu.custom_ops.precompiled_user_op(
            [_params, _indices],
            library_path=str(
                Path(__file__).parent / "custom_grouped_gather_scatter.so"
            ),
        ...
        def grad(upstream: tf.Tensor) -> (tf.Tensor, None):
            return grouped_scatter(upstream, _indices, table_size=table_size), None
...
def grouped_scatter(data: tf.Tensor, indices: tf.Tensor, table_size: int) -> (tf.Tensor, Callable):
