...
Settings parseAndPlan(const std::string& spec, poplar::Graph& graph) {
    ...
    if (version != "v0" || !in.eof() || in.fail()) {
        ...
        msg << "Bad attribute '" << spec
            << "', expected 'v0 <operation> \"float\"|\"half\" <nGroups> <tableSize> "
               "<embeddingSize> <lookupSize>'";
        throw std::invalid_argument(msg.str());
    }

    settings.plan =
        popops::embedding::plan(graph, settings.type, settings.nGroups, settings.tableSize,
...
extern "C" poplar::Tensor Build_allocator(poplar::Graph& graph,
    ...
    if (operand == 0 && settings.operation == Operation::Scatter) {
        return popops::createGroupedSliceTensor(graph, settings.type, settings.nGroups,
                                                {settings.tableSize, settings.embeddingSize}, {0},
                                                {1}, settings.lookupSize, settings.plan,
                                                settings.options, debugPrefix)
            .squeeze({2});
    }
    if (operand == 1) {
        return popops::createGroupedIndicesTensor(graph, settings.nGroups, {0}, settings.lookupSize,
...
extern "C" poplar::program::Program Build(poplar::Graph& graph,
    ...
    if (settings.operation == Operation::Gather) {
        ...
        auto output = popops::groupedMultiSlice(graph, params, indices, {0}, {1}, program,
                                                settings.plan, settings.options, debugPrefix);
        outputs = {output.squeeze({2})};
    }
    if (settings.operation == Operation::Scatter) {
