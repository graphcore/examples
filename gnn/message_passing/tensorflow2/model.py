...
def create_model():
    ...
    if FLAGS.gather_scatter == 'dense':
        inputs_list.extend([
            # receivers
            tf.keras.Input((FLAGS.n_edges_per_pack, FLAGS.n_nodes_per_pack), dtype=tf.int32,
                           batch_size=FLAGS.micro_batch_size),
            # senders
            tf.keras.Input((FLAGS.n_edges_per_pack, FLAGS.n_nodes_per_pack), dtype=tf.int32,
                           batch_size=FLAGS.micro_batch_size),
            # node graph idx
            tf.keras.Input((FLAGS.n_nodes_per_pack, FLAGS.n_graphs_per_pack),
                           dtype=tf.int32, batch_size=FLAGS.micro_batch_size),
        ])
    else:
        inputs_list.extend([
            # receivers
