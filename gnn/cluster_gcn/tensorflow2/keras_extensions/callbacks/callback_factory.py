...
class CallbackFactory:
    ...
    def get_callbacks(
        ...
        callbacks.append(
            BatchStatisticsCallback(num_nodes_processed_per_execution, real_over_padded_ratio, total_num_epochs, loss)
        )
        ...
        # For distributed training, perform allreduce on metrics
        if distributed_training:
            callbacks.append(AllReduceMetricsCallback())

        if log_period > 0:
            logging.info(
                "Creating callback for logging to terminal with" f" a period of 1 log every {log_period} executions."
            )
            ...
            if config["wandb"] and popdist.getInstanceIndex() == 0:
                ...
                callbacks.append(TrainingCustomWandbCallback(name=universal_run_name, config=config))
                callbacks.append(ValidationCustomWandbCallback(name=universal_run_name, config=config))
        if popdist.getInstanceIndex() == 0:
            logging.info(
                "Creating callback for creating checkpoints. Checkpoints"
                f" will be saved at a period of 1 every {executions_per_ckpt}"
                f" executions to path: {checkpoint_path}"
            )
            callbacks.append(
                CheckpointCallback(checkpoint_dir=checkpoint_path, executions_per_ckpt=executions_per_ckpt)
            )
