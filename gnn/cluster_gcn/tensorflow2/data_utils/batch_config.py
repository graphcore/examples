...
class BatchConfig:
    def __init__(
        ...
        if self.num_real_nodes_per_epoch is not None:
            ...
            self.real_over_padded_ratio = (
                self.num_real_nodes_per_epoch * self.epochs_per_execution
            ) / num_nodes_per_epoch
        else:
        ...
        steps_per_execution_per_replica = max(
            1, all_instance_steps_per_epoch // (self.num_replicas * self.executions_per_epoch)
        )
    ...
    def global_batch_size(self):
        return self.micro_batch_size * self.gradient_accumulation_steps_per_replica * self.num_replicas

    @property
    def num_nodes_processed_per_execution(self):
        return self.micro_batch_size * self.max_nodes_per_batch * self.steps_per_execution * self.num_replicas

    @property
    def num_clusters_processed_per_execution(self):
        return self.micro_batch_size * self.clusters_per_batch * self.steps_per_execution * self.num_replicas
