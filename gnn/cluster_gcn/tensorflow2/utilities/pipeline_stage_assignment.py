...
def get_poplar_options_per_pipeline_stage(
    ...
    if len(matmul_available_memory_proportion) != num_ipus_per_replica:
        raise ValueError(
            "Available memory proportion must be set for each of" f" the {num_ipus_per_replica} IPUs in the pipeline."
        )

    return [
        ipu.pipelining_ops.PipelineStageOptions(
            matmul_options=get_matmul_options(matmul_available_memory_proportion[stage], matmul_partials_type)
        )
...
def pipeline_model(
    model, config, pipeline_names, pipeline_allocate_previous, num_ipus_per_replica, matmul_partials_type
):
    """
