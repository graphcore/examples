...
def get_loss_and_metrics(task, num_labels, adjacency_form, metrics_precision, enable_loss_outfeed=True):
    ...
    if task == Task.BINARY_MULTI_LABEL_CLASSIFICATION:
        logging.info("Using loss and accuracy for binary multi" " label classification task.")
        return get_loss_accuracy_f1score_for_binary_case(
            num_labels, adjacency_form, metrics_precision, enable_loss_outfeed=enable_loss_outfeed
        )
    elif task == Task.MULTI_CLASS_CLASSIFICATION:
        logging.info("Using loss and accuracy for multi" " class classification task.")
...
def get_loss_accuracy_f1score_for_binary_case(num_labels, adjacency_form, metrics_precision, enable_loss_outfeed=True):
    """Given the number of labels, returns the loss function, accuracy and
    F1 score metrics for binary multi label classification."""
    loss_fn_class = get_masked_loss_class(tf.keras.losses.BinaryCrossentropy, activation=tf.math.sigmoid)
    if enable_loss_outfeed:
        ...
        # an outfeed queue.
        loss_fn_class = wrap_loss_in_enqueuer(loss_fn_class, ["loss_instantaneous"])
    ...
    accuracy_metric_class = get_masked_accuracy_class(tf.keras.metrics.BinaryAccuracy, activation=tf.math.sigmoid)
    accuracy = accuracy_metric_class(adjacency_form=adjacency_form, name="accuracy_epoch_avg", dtype=metrics_precision)
...
def get_loss_accuracy_f1score_for_categorical_case(
    num_labels, adjacency_form, metrics_precision, enable_loss_outfeed=True
):
    """Given the number of labels, returns the loss function, accuracy and F1
    score metrics for multi class classification."""

    loss_fn_class = get_masked_loss_class(tf.keras.losses.SparseCategoricalCrossentropy)
    ...
    accuracy_metric_class = get_masked_accuracy_class(tf.keras.metrics.SparseCategoricalAccuracy)
    accuracy = accuracy_metric_class(adjacency_form=adjacency_form, name="accuracy_epoch_avg", dtype=metrics_precision)
...
def set_masked_elements_to_zero(labels, mask):
    ...
    # as labels
    mask_repeated = tf.repeat(tf.reshape(mask, (-1, 1)), labels.shape[-1], axis=1)
    # Apply mask to labels to make the masked values `0`.
    labels = labels * tf.cast(mask_repeated, labels.dtype)
    return labels


def get_masked_loss_class(loss_class, activation=tf.keras.activations.linear):
    ...
    class MaskedLoss(loss_class):
        def __init__(self, adjacency_form, dtype=tf.float32, *args, **kwargs):
            if kwargs["reduction"] != tf.keras.losses.Reduction.NONE:
                raise ValueError("In order to correctly apply the mask" " the reduction type must be NONE.")
        ...
        def call(self, y_true, y_pred):
            ...
            loss *= mask_cast
            return tf.math.divide_no_nan(tf.reduce_sum(loss), tf.reduce_sum(mask_cast))

    return MaskedLoss


def get_masked_accuracy_class(accuracy_metric_class, activation=tf.keras.activations.linear):
    ...
    class MaskedAccuracy(accuracy_metric_class):
        ...
        def update_state(self, y_true, y_pred, sample_weight=None):
            ...
            super().update_state(y_true, y_pred, sample_weight=tf.cast(mask, y_pred.dtype))
...
class MaskedF1Score(tfa.metrics.F1Score):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(dtype=metrics_precision, *args, **kwargs)
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        if self.labels_to_one_hot:
            y_true = tf.one_hot(tf.reshape(y_true, [-1]), depth=self.num_classes, dtype=self.metrics_precision)
        # Apply activation to the prediction
        y_pred = self.activation(y_pred)
        # Update the metric state with the mask as sample_weight
        super().update_state(y_true, y_pred, sample_weight=tf.cast(mask, self.metrics_precision))
