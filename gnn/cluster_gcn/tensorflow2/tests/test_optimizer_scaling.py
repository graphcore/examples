...
def get_model_with_grad_accum(grad_accum):
    ...
    model.set_gradient_accumulation_options(gradient_accumulation_steps_per_replica=grad_accum)
    return model
...
def get_pipelined_model(grad_accum):
    ...
    model.set_pipelining_options(gradient_accumulation_steps_per_replica=grad_accum)
    return model


def ipu_prog(num_elements, num_replicas, gradient_accumulation, pipeline, loss_scaling):
    micro_batch_size = int(num_elements / gradient_accumulation / num_replicas)

    ds = tf.data.Dataset.from_tensor_slices(
        (np.array([1.0] * num_elements, dtype=np.float32), np.array([2.0] * num_elements, dtype=np.float32))
    )
...
class TestGradientNormalization:
    ...
    def test_scaling_in_optimizer(self, replicas, grad_accum, pipeline, loss_scaling):
        weight = ipu_prog(
            num_elements=16,
