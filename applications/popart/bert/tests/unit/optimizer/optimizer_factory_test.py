...
class TestConfig(NamedTuple):
    ...
    # How to control the learning rate. Choices: "Scheduled", "Linear"
    learning_rate_function: str = "Scheduled"
    ...
    lr_warmup_start: int = 1e-7
    lr_warmup_steps: int = 100
    lr_steps_per_warmup_update: int = 1
...
@pytest.mark.parametrize(
    ...
    [
        (2, {0: 0.5, 1: 0.1}, [100, 200, 300], True),
        (5, {0: 0.0008, 3: 0.00001}, [5, 3, 7], True),
        (5, {0: 0.0008, 3: 0.00001}, [5, 3, 7], False)
    ])
def test_per_tensor_lr(steps_per_epoch, lr_schedule_by_step, layer_inputs, pipeline):
    ...
    def generate_test_case(step_schedule, layer_inputs, pipeline=True):
    ...
    def assert_scaled_lr(factory, true_scaling_values):
        ...
        if len(factory.transforms) > 0:
            for stage, scale_value in enumerate(true_scaling_values):
                assert scale_value == factory.transforms[0]("", {"learningRate": 1}, stage)["learningRate"]
    ...
    test_case, config, true_scaling = generate_test_case(lr_schedule_by_step,
                                                         layer_inputs,
                                                         pipeline=pipeline)
