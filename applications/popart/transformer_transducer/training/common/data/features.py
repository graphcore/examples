...
def stack_subsample_frames(x, x_lens, stacking=1, subsampling=1):
    ...
    if subsampling > 1:
        ...
        if x.size(2) > x_lens.max().item():
            # this assert from reference seems unnecessary
            # commenting out so that training with synthetic data works
...
class FillPadding(BaseFeatures):
    ...
    def __init__(self, optim_level, max_seq_len, fill_value=0):
    ...
    def calculate_features(self, x, x_lens):
        ...
        in_len = x.size(-1)
        mask = torch.arange(in_len, dtype=x_lens.dtype, device=x.device)
        mask = mask.expand(x.size(0), in_len) >= x_lens.unsqueeze(1)
        x = x.masked_fill(mask.unsqueeze(1), self.fill_value)

        if self.max_seq_len > in_len:
            padding_size = list(x.size())
            padding_size[-1] = self.max_seq_len - in_len
            padding = torch.ones(padding_size, dtype=x.dtype) * self.fill_value
