...
class DaliDataLoader:
    ...
    def __init__(self, gpu_id, dataset_path: str, config_data: dict, config_features: dict, json_names: list,
        ...
        if pipeline_type == 'train':
            min_speed_perturbation = config_data['speed_perturbation']['min_rate']
            self._max_spec_len_before_stacking = round((max_duration / min_speed_perturbation) / window_stride)
        elif pipeline_type == 'val':
            # no speed perturbation is applied for validation
            self._max_spec_len_before_stacking = round(max_duration / window_stride)
    ...
    @property
    def max_spec_len_before_stacking(self):
        return self._max_spec_len_before_stacking

    @property
    def min_token_sequence_len(self):
        return self._dali_data_iterator.min_token_sequence_len

    @property
    def max_token_sequence_len(self):
        return self._dali_data_iterator.max_token_sequence_len
