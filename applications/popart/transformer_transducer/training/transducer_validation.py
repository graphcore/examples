...
def create_inputs_for_inference(builder, model_conf, conf):
    ...
    in_feats = model_conf["rnnt"]["in_feats"]  # num-mel-bands X frame-stacking-factor
...
def create_model_and_dataflow_for_inference(builder, model_conf, conf, inputs):
    ...
    inference_transcription_out, inference_transcription_out_lens = transcription_network(inputs["mel_spec_input"],
                                                                                          inputs["input_length"])
    logger.info("Shape of Transcription-Network Output: {}".format(builder.getTensorShape(inference_transcription_out)))
    ...
    inference_transcription_out_len = builder.getTensorShape(inference_transcription_out)[1]
    ...
    with builder.virtualGraph(0):
        inference_transcription_out = joint_network.joint_transcription_fc(inference_transcription_out)
    logger.info("Shape of Transcription-Network Output after Joint-transciption-fc: {}".format(builder.getTensorShape(inference_transcription_out)))
...
def setup_validation_data_pipeline(conf, transducer_config):
    ...
    val_manifests = [os.path.join(conf.data_dir, "librispeech-dev-clean-wav.json")]
    ...
    val_dataset_kw, val_features_kw, val_splicing_kw, val_specaugm_kw = config.input(transducer_config, "val")

    sampler = IpuSimpleSampler(conf.samples_per_step, conf.num_instances, conf.instance_idx)

    assert(conf.samples_per_step % conf.num_instances == 0)
    samples_per_step_per_instance = conf.samples_per_step // conf.num_instances
    logger.debug("DaliDataLoader SamplesPerStepPerInstance = {}".format(samples_per_step_per_instance))
    ...
    conf.max_spec_len_after_stacking = round(val_loader.max_spec_len_before_stacking / val_splicing_kw["frame_subsampling"])
    conf.num_symbols = val_tokenizer.num_labels + 1

    val_feat_proc = torch.nn.Sequential(
        val_specaugm_kw and features.SpecAugment(optim_level=0, **val_specaugm_kw) or torch.nn.Identity(),
...
def update_pytorch_rnnt_model(pytorch_rnnt_model, decoder_weights_path):
    ...
    assert(list(embedding_module.weight.shape) == list(embedding_weight_pytorch.shape))
    embedding_module.weight = torch.nn.parameter.Parameter(torch.tensor(embedding_weight_pytorch))

    lstm_module = pytorch_rnnt_model.prediction.dec_rnn.lstm

    for layer in range(lstm_module.num_layers):
        lstm_input_weights_key = "prediction_net_rnn_{}/lstm_input_weights".format(layer)
        lstm_output_weights_key = "prediction_net_rnn_{}/lstm_output_weights".format(layer)
        lstm_biases_key = "prediction_net_rnn_{}/lstm_biases".format(layer)
        lstm_ih_weight_pytorch = convert_lstm_weight(decoder_weights_dict[lstm_input_weights_key])
        lstm_hh_weight_pytorch = convert_lstm_weight(decoder_weights_dict[lstm_output_weights_key])
        lstm_ih_bias_pytorch, lstm_hh_bias_pytorch = convert_lstm_bias(decoder_weights_dict[lstm_biases_key])
        ...
        lstm_module.__setattr__(weight_ih_key, torch.nn.parameter.Parameter(torch.tensor(lstm_ih_weight_pytorch)))
        ...
        lstm_module.__setattr__(weight_hh_key, torch.nn.parameter.Parameter(torch.tensor(lstm_hh_weight_pytorch)))
        ...
        lstm_module.__setattr__(bias_ih_key, torch.nn.parameter.Parameter(torch.tensor(lstm_ih_bias_pytorch)))
        ...
        lstm_module.__setattr__(bias_hh_key, torch.nn.parameter.Parameter(torch.tensor(lstm_hh_bias_pytorch)))

    joint_pred_module_fc = pytorch_rnnt_model.joint_pred

    joint_pred_fc_weight_pytorch = convert_fc_weight(decoder_weights_dict["joint_net_prediction_fc/weights"])
    joint_pred_fc_bias_pytorch = convert_fc_bias(decoder_weights_dict["joint_net_prediction_fc/bias"])

    assert(list(joint_pred_module_fc.weight.shape) == list(joint_pred_fc_weight_pytorch.shape))
    joint_pred_module_fc.weight = torch.nn.parameter.Parameter(torch.tensor(joint_pred_fc_weight_pytorch))
    assert(list(joint_pred_module_fc.bias.shape) == list(joint_pred_fc_bias_pytorch.shape))
    joint_pred_module_fc.bias = torch.nn.parameter.Parameter(torch.tensor(joint_pred_fc_bias_pytorch))

    joint_out_module_fc = pytorch_rnnt_model.joint_net[2]

    joint_out_fc_weight_pytorch = convert_fc_weight(decoder_weights_dict["joint_net_out_fc/weights"])
    joint_out_fc_bias_pytorch = convert_fc_bias(decoder_weights_dict["joint_net_out_fc/bias"])

    assert(list(joint_out_module_fc.weight.shape) == list(joint_out_fc_weight_pytorch.shape))
    joint_out_module_fc.weight = torch.nn.parameter.Parameter(torch.tensor(joint_out_fc_weight_pytorch))
    assert(list(joint_out_module_fc.bias.shape) == list(joint_out_fc_bias_pytorch.shape))
    joint_out_module_fc.bias = torch.nn.parameter.Parameter(torch.tensor(joint_out_fc_bias_pytorch))
...
def evaluate(conf, onnx_path, val_loader, val_feat_proc,
    ...
    logger.info("Getting trained weights for inference from {}".format(onnx_path))
    inference_session.resetHostWeights(onnx_path, ignoreWeightsInModelWithoutCorrespondingHostWeight=True)
    ...
    for audio, audio_lens, txt, txt_lens in val_loader:
        ...
        feat_lens = torch.tensor(inference_anchors[inference_transcription_out_lens])
    ...
    logger.info("Creating multiprocessor pool with {} workers and function for decoding".format(num_workers))
    greedy_decoding_processor_pool = multiprocessing.pool.ThreadPool(processes=num_workers)
    ...
    for feats, feat_lens, txt, txt_lens in feat_iter:
        ...
        for bind in range(conf.batches_per_step):
            ...
            pred_results.append(greedy_decoding_processor_pool.apply_async(greedy_decoding_func,
                                                                           (feats_b, feat_lens_b)))
            ground_truths_dekotenized.append(helpers.gather_transcripts([txt_b], [txt_lens_b], detokenize))
    ...
    for idx, (pred_result, gts_detokenized) in enumerate(pred_iter):
        preds_detokenized = helpers.gather_predictions([pred_result.get()], detokenize)

        batch_wer, batch_scores, batch_words = metrics.word_error_rate(preds_detokenized, gts_detokenized)

        agg['preds'] += preds_detokenized
        agg['txts'] += gts_detokenized

    wer, scores, num_words, _ = helpers.process_evaluation_epoch(agg)
    logger.info("Total time for Transducer Decoding = {:.1f} secs".format(time.time() - start_time))
...
def dist_wer(scores, num_words):
    ...
    comm = MPI.COMM_WORLD
    scores = comm.allreduce(scores, op=MPI.SUM)
    num_words = comm.allreduce(num_words, op=MPI.SUM)
    if num_words != 0:
...
if __name__ == "__main__":
    ...
    val_loader, val_feat_proc, val_tokenizer = setup_validation_data_pipeline(runtime_conf, transducer_config)

    pytorch_rnnt_model = create_pytorch_rnnt_model(transducer_config, val_tokenizer.num_labels + 1)

    greedy_decoder = TransducerGreedyDecoder(blank_idx=0, max_symbols_per_step=conf.max_symbols_per_step, shift_labels_by_one=True)

    device = device_module.acquire_device(conf, runtime_conf.local_replication_factor, runtime_conf)
    inference_session, inference_anchors, inference_inputs, \
        inference_transcription_out, inference_transcription_out_lens = \
        create_inference_transcription_session(device, conf.model_conf, runtime_conf)

    decoder_weights_path = os.path.join(conf.model_dir, "decoder_weights_validation.npy")
