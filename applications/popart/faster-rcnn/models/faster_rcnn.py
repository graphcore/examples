...
import numpy as np
import json
from IPU.basic_func import np64to32_or_16
from layer.rpn import RPN
from layer.vgg import VGG16
...
class FasterRcnn(BaseModel):
    ...
    def get_loss_info(self, outputs_dic):
        for i, loss_name in enumerate(self.loss_names):
            local_loss = outputs_dic[loss_name].data
            local_moving_loss = self.moving_loss[loss_name]
            local_moving_loss.update(local_loss)

        info = ' >>> total loss: %.6f(%.6f)\n >>> rpn_loss_cls: %.6f(%.6f)\n >>> rpn_loss_box: %.6f(%.6f)\n >>> loss_cls: %.6f(%.6f)\n >>> loss_box: %.6f(%.6f)' % (
            outputs_dic['loss'].data, self.moving_loss['loss'].var,
            outputs_dic['rpn_loss_cls'].data,
            self.moving_loss['rpn_loss_cls'].var,
            outputs_dic['rpn_loss_bbox'].data,
            self.moving_loss['rpn_loss_bbox'].var,
            outputs_dic['rcnn_loss_cls'].data,
            self.moving_loss['rcnn_loss_cls'].var,
            outputs_dic['rcnn_loss_bbox'].data,
