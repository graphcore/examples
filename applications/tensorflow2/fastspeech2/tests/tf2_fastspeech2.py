...
class TFTacotronPostnet(tf.keras.layers.Layer):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.activation = [tf.nn.tanh] * \
            (config.n_conv_postnet - 1) + [tf.identity]
        ...
        for i in range(config.num_duration_conv_layers):
            ...
            self.conv_layers.append(
                tf.keras.layers.LayerNormalization(
                    epsilon=config.layer_norm_eps, name="LayerNorm_._{}".format(
                        i)
                )
            )
            ...
            self.conv_layers.append(
                tf.keras.layers.Dropout(
                    config.duration_predictor_dropout_probs)
            )
...
class TFFastSpeechLengthRegulator(tf.keras.layers.Layer):
    ...
    def __init__(self, config, **kwargs):
        """Init variables."""
        self.enable_tflite_convertible = kwargs.pop(
    ...
    def _length_regulator(self, encoder_hidden_states, durations_gt):
        ...
        if self.enable_tflite_convertible:
            ...
            repeat_encoder_hidden_states = tf.expand_dims(
                tf.pad(repeat_encoder_hidden_states,
                       [[0, pad_size], [0, 0]]), 0
...
class TFFastSpeech(tf.keras.Model):
    ...
    def call(
        ...
        # duration predictor, here use last_encoder_hidden_states, you can use more hidden_states layers
        ...
        decoder_output = self.decoder(
            [length_regulator_outputs, speaker_ids,
        ...
        mel_after = (
            self.postnet([mel_before, encoder_masks],
                         training=training) + mel_before
        )
        ...
        self.inference = tf.function(
            ...
            input_signature=[
                ...
                tf.TensorSpec(shape=[None, ],
                              dtype=tf.int32, name="speaker_ids"),
                tf.TensorSpec(shape=[None, ],
                              dtype=tf.float32, name="speed_ratios"),
            ],
        )
...
class TFFastSpeech2(TFFastSpeech):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.f0_dropout = tf.keras.layers.Dropout(
            config.variant_predictor_dropout_rate)
        ...
        self.energy_dropout = tf.keras.layers.Dropout(
            config.variant_predictor_dropout_rate)
    ...
    def call(
        ...
        **kwargs,
        ...
        f0_outputs = self.f0_predictor(
            [last_encoder_hidden_states, speaker_ids,
        ...
        mels_after = (
            self.postnet([mels_before, encoder_masks],
                         training=training) + mels_before
        )
        ...
        self.inference = tf.function(
            ...
            input_signature=[
                ...
                tf.TensorSpec(shape=[None, ],
                              dtype=tf.float32, name="speed_ratios"),
                tf.TensorSpec(shape=[None, ],
                              dtype=tf.float32, name="f0_ratios"),
                tf.TensorSpec(shape=[None, ],
                              dtype=tf.float32, name="energy_ratios"),
            ],
        )
