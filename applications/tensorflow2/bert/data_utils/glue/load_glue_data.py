...
def get_encodings_from_example(example, tokenizer, task_keys, return_offsets_mapping=False):
...
def convert_example_to_tf_features(example, tokenizer, task_keys):
    encodings, label, idx = get_encodings_from_example(example, tokenizer, task_keys)
    return encodings, {'labels': label}


def convert_to_features(dataset, tokenizer, convert_example_func, task_keys):
    labels = {}
    features = {}
    for item in tqdm(dataset):
        feature, label = convert_example_func(
            item, tokenizer, task_keys)
...
def get_glue_data(task, micro_batch_size, cache_dir, generated_dataset=False):
    task_to_keys = {
        "cola": ("sentence", None),
