...
def get_real_dataset(
    ...
    if is_training:
        ...
        if distributed_worker_count > 1:
            d = d.shard(num_shards=distributed_worker_count,
    ...
    d = d.map(lambda record: _decode_record(record,
                                            name_to_features,
                                            data_type,
                                            debug,
                                            test))
    d = d.batch(batch_size=micro_batch_size,
                drop_remainder=True)
...
def get_generated_dataset(
    ...
    distributed_worker_index,
        ...
        _masked_lm_positions = np.random.randint(max_seq_length, size=(generated_dataset_size, max_predictions_per_seq)).astype(np.int32)
