...
# This script can be used to pretrain BERT base or large on IPU-POD64 and IPU-POD128

# Directories of sdk, ven, dataset, and profile folder
# ------------------------------------
SDK_PATH="/localdata/${USER}/sdks"
VENV_PATH="/localdata/${USER}/venvs"
WIKIPEDIA_DATA_PATH="/a/scratch/ai-datasets/preprocessed/wikipedia/128"
...
# -------------
display_usage() {
	echo "Pretrain BERT on IPU-POD64 or IPU-POD128"
...
POPRUN_OPTIONS="-vv --host ${HOSTS_LIST} --num-ilds ${NUM_ILDS} --num-instances ${NUM_INSTANCES} --num-replicas ${NUM_REPLICAS} --ipus-per-replica 4 --vipu-server-host=${VIPU_SERVER_IP} --vipu-cluster=${VIPU_CLUSTER_NAME} --reset-partition=yes --update-partition=yes --remove-partition=no --vipu-partition=${VIPU_PARTITION_NAME} --print-topology=yes"
