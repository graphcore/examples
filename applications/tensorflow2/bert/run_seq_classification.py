...
from model.pipeline_stage_names import (PIPELINE_ALLOCATE_PREVIOUS,
                                        PIPELINE_NAMES)
from utilities.argparser import (add_shared_arguments, add_glue_arguments,
...
from utilities.ipu_utils import (create_ipu_strategy,
...
def fine_tune_glue(config):
    ...
    if config.bert_model_name is None and config.pretrained_ckpt_path is None:
        logging.warning("GLUE requires a pretrained model, either"
    ...
    # =================================
    dataset, eval_dataset, test_dataset, num_train_samples, \
        num_eval_samples, num_test_samples, _ = get_glue_data(
            config.glue_task, config.global_batch.micro_batch_size, config.dataset_dir,
    ...
    train_config = BatchConfig(micro_batch_size=config.global_batch.micro_batch_size,
    ...
    strategy = create_ipu_strategy(num_ipus_per_replica=num_ipus_per_replicas,
                                   num_replicas=config.global_batch.replicas,
    ...
    with strategy.scope():
        if config.bert_model_name is not None:
            ...
            bert_config = BertConfig(
                **config.bert_config.dict(), hidden_act=ipu.nn_ops.gelu)
        ...
        if config.pretrained_ckpt_path:
            ...
            _ = load_checkpoint_into_model(
                model=model,
                pretrained_ckpt_path=config.pretrained_ckpt_path,
                expect_partial=not config.do_training)

        # Configure pipeline stages
        # =========================
        if num_pipeline_stages > 1:
            pipeline_assigner = GluePipelineStagesAssigner(
                PIPELINE_ALLOCATE_PREVIOUS, PIPELINE_NAMES)
            assignments = model.get_pipeline_stage_assignment()
            assignments = pipeline_assigner.assign_glue_pipeline_stages(
                assignments, config.ipu_config.pipeline_stages)
        ...
        loss_function = (ClassificationLossFunctionRegression
                         if config.glue_task == "stsb" else ClassificationLossFunction)
        ...
        if config.glue_task != "stsb":
            ...
            accuracy = wrap_stateless_metric_in_enqueuer("glue_accuracy",
                                                         classification_accuracy_fn,
                                                         ["glue_accuracy"])
        ...
        if config.do_training:
            ...
            optimizer = get_optimizer(
                config.optimizer_opts.name,
                config.global_batch.grad_acc_steps_per_replica,
                config.global_batch.replicas,
                lr_scheduler,
                config.use_outlining,
            ...
            model.fit(
                dataset,
                steps_per_epoch=train_config.num_micro_batches_per_epoch,
        ...
        if config.do_validation:
            ...
            if (train_config.steps_per_execution != eval_batch_config.steps_per_execution or
                    not config.do_training):
                model.compile(
                    metrics=[accuracy],
                    steps_per_execution=eval_batch_config.steps_per_execution)
        ...
        if config.do_test:
            test_batch_config = BatchConfig(micro_batch_size=config.global_batch.micro_batch_size,
            ...
            if (train_config.steps_per_execution != test_batch_config.steps_per_execution or
                    not config.do_training):
                model.compile(
                    steps_per_execution=test_batch_config.steps_per_execution)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="TF2 BERT GLUE Fine Tuning",
