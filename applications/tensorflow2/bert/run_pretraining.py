...
def pretrain(config):
    ...
    # Load training data
    # ==================
    dataset, num_samples = get_pretraining_dataset(micro_batch_size=config.global_batch.micro_batch_size,
    ...
    batch_config = BatchConfig(micro_batch_size=config.global_batch.micro_batch_size,
    ...
    with strategy.scope():
        ...
        if config.pretrained_ckpt_path:
            ...
            ckpt_found = load_checkpoint_into_model(
                model,
                config.pretrained_ckpt_path)

        if task == Task.PRETRAIN_PHASE_TWO and not ckpt_found:
            logging.warning("WARNING: Phase 2 pre-training should be done from a completed Phase 1 checkpoint. "
        ...
        outfeed_queues = [lr_outfeed_queue,
                          nsp_loss.outfeed_queue,
                          mlm_loss.outfeed_queue]
        ...
        if config.show_accuracy:
            pretraining_accuracy = wrap_stateless_metric_in_enqueuer(
                "accuracy",
                pretraining_accuracy_fn,
                ["mlm_acc_average", "nsp_acc_average"])
            outfeed_queues.append(pretraining_accuracy.outfeed_queue)

        # Prepare optimizer.
        outline_optimizer_apply_gradients = False if config.ipu_config.replicated_tensor_sharding else config.use_outlining
        optimizer = get_optimizer(
            optimizer_name=config.optimizer_opts.name,
            gradient_accumulation_factor=config.global_batch.grad_acc_steps_per_replica,
        ...
        logging.info("Forcing `model.fit` to run a particular number of steps by"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="TF2 BERT Pretraining",
