...
def partition_graph(adj,
    ...
    logging.info(f"Partitioning done. {time.time() - start_time} seconds.")

    num_nodes_per_cluster = sorted([len(pt) for pt in parts])
    max_nodes_per_batch = sum(num_nodes_per_cluster[-clusters_per_batch:])

    logging.info(f"Inferred max nodes per batch {max_nodes_per_batch}"
                 f", given number of clusters ({num_clusters}) and"
                 f" clusters per batch ({clusters_per_batch}).")

    return parts, max_nodes_per_batch
