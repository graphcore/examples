...
class TestLossEnqueuer:
    ...
    def test_single_loss_enqueued(self):
        ...
        loss = wrap_metric_in_enqueuer(tf.keras.losses.CategoricalCrossentropy,
                                       loss_outfeed_queue,
                                       [loss_key])()
        history = ipu_prog(loss)

        assert loss_outfeed_queue.enqueued
        x = loss_outfeed_queue.dequeue()
    ...
    def test_multiple_loss_enqueued_from_multiple_loss_functions(self):
        loss_key_1 = "loss_1"
        loss_outfeed_queue_1 = ipu.ipu_outfeed_queue.IPUOutfeedQueue()
        loss_1 = wrap_metric_in_enqueuer(tf.keras.losses.CategoricalCrossentropy,
                                         loss_outfeed_queue_1,
                                         [loss_key_1])()
        loss_key_2 = "loss_2"
        loss_outfeed_queue_2 = ipu.ipu_outfeed_queue.IPUOutfeedQueue()
        loss_2 = wrap_metric_in_enqueuer(tf.keras.losses.CategoricalCrossentropy,
                                         loss_outfeed_queue_2,
                                         [loss_key_2])()
        history = ipu_prog({"dense_0": loss_1, "dense_1": loss_2}, heads=2)

        assert loss_outfeed_queue_1.enqueued
        x = loss_outfeed_queue_1.dequeue()
        assert loss_key_1 in x
        np.testing.assert_allclose(history.history["dense_0_loss"][0], x[loss_key_1][0][-1])

        assert loss_outfeed_queue_2.enqueued
        x = loss_outfeed_queue_2.dequeue()
        assert loss_key_2 in x
        np.testing.assert_allclose(history.history["dense_1_loss"][0], x[loss_key_2][0][-1])
