...
from schedules.decorators import LearningRateEnqueuer, ShiftWarmup, FadingMaskWarmup, StairCase
...
SUPPORTED_LR_SCHEDULES = {
    'cosine': (CosineLRSchedule, {
        ...
        'epochs_to_total_decay': 1.1
    }),
    'stepped': (PiecewiseConstantDecay, {
        'boundaries': [0.1, 0.5, 0.8],
        'values': [0.00001, 0.0001, 0.0005, 0.00001]
    }),
...
class LearningRateSchedulesTest(unittest.TestCase):
    ...
    def test_getting_supported_lr_schedules(self):
        for scheduler_name, (cls, params) in SUPPORTED_LR_SCHEDULES.items():
            schedule = get_lr_scheduler(scheduler_name=scheduler_name,
    ...
    def test_error_for_unsupported_lr_schedules(self):
        try:
            get_lr_scheduler(scheduler_name='hygge',
                             schedule_params={},
                             global_batch_size=1,
                             weight_updates_per_epoch=1)
        except NameError:
    ...
    def test_lr_scheduler_builder_factor(self):
        scheduler = get_lr_scheduler(scheduler_name='const',
...
class ScheduleDecoratorsTest(unittest.TestCase):
    ...
    def test_enqueue_decorator(self):
        ...
        enqueued_schedule = LearningRateEnqueuer(self.schedule, queue=outfeed_queue)

        @tf.function(experimental_compile=True)
        def fn():
            enqueued_schedule(10)

        self.run_fn_on_device(fn)
        assert outfeed_queue.dequeue() == 10 + self.schedule.initial_learning_rate
    ...
    def test_fading_mask_warmup_decorator(self):
        ...
        assert scheduler_with_warmup(0) == self.schedule.initial_learning_rate / 10
        assert scheduler_with_warmup((warmup_weight_updates - 1) /
                                     2) == self.schedule((warmup_weight_updates - 1) / 2) / 2
    ...
    def test_staircase_decorator(self):
        ...
        for step in range(num_steps):
            self.assertEqual(schedule_with_staircase(step),
