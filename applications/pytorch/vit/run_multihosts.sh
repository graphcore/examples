...
then
    echo "8 hosts are specified to use POD128."
    ilds=2
    replicas=32
    instances=8
    accumulation=146
    batch_size=14
    rebatched_worker_size=1168
    workers=64
    iterations=512
elif [[ $num -eq 15 ]]
then
    echo "16 hosts are specified to use POD256."
    ilds=4
    replicas=64
    instances=16
    accumulation=73
    batch_size=14
    rebatched_worker_size=584
    workers=64
    iterations=1024
else
...
poprun --host $hosts \
      ...
      --num-ilds=$ilds \
      --mpi-global-args="--tag-output --allow-run-as-root --output-filename output_poprun --mca oob_tcp_if_include $interface1 --mca btl_tcp_if_include $interface2" \
