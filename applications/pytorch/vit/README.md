...
Shell scripts that wrap up above arguments of the cluster setup and the python launcher configurations to run on POD16, POD64, POD128 and POD256 are provided. Use `bash run_singlehost.sh` or `bash run_multihosts.sh` to see meaning of the arguments. Specifically, these arguments are listed in the table below.
...
```
Similarly, using `run_multihosts.sh`, 8/16 hosts are needed for running on POD128/POD256.
