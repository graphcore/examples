...
def test_single_value_layers_per_ipu():
    ...
    """.split()
    config = BertConfig(**(vars(parse_bert_args(args))))
    assert config.layers_per_ipu == [1, 1, 1]
...
def test_multi_value_layers_per_ipu():
    ...
    --config unit_test
    --encoder-start-ipu 1
    --layers-per-ipu 3 3 4
    --num-hidden-layers 10
    """.split()
    config = BertConfig(**(vars(parse_bert_args(args))))
    assert config.layers_per_ipu == [3, 3, 4]
...
def test_get_layer_ipu():
    ...
    """.split()
    config = BertConfig(**(vars(parse_bert_args(args))))
    assert (_get_layer_ipu(config.layers_per_ipu, config.encoder_start_ipu) ==
