...
class MultiCropWrapper(nn.Module):
    ...
    def set_pipeline(self):
        ...
        self.student_head.last_layer = poptorch.BeginBlock(
            self.student_head.last_layer, f'ipu{last_id}', ipu_id=last_id)
        self.teacher_head.last_layer = poptorch.BeginBlock(
            self.teacher_head.last_layer, f'ipu{last_id}', ipu_id=last_id)
    ...
    def _momentum_update_pipeline(self, ema_factor):
        ...
        self.single_ema_update(
            ...
            last_id - 1)
        self.ema_update(
            self.student_head.last_layer.weight_v,
            self.teacher_head.last_layer.weight_v,
            ema_factor,
            last_id)
