...
# BatchNormWithGlobalNormalization and FusedBatchNormV2.  The two types only
...
def fold_batch_norms(input_graph_def):
    ...
    FusedBatchNormV2 ops, and will need to be extended in the future to handle the
    newer style.
    ...
    new_ops = []
    for node in input_graph_def.node:
        if node.op not in ("BatchNormWithGlobalNormalization", "FusedBatchNormV2", "FusedBatchNormV3"):
            continue
