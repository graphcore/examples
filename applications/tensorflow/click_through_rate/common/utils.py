...
def calc_auc(raw_arr):
    ...
    for record in arr:
        ...
        if neg != 0 and pos != 0:
            ...
            xy_arr.append([0, tp/pos])
        elif neg != 0 and pos == 0:
            xy_arr.append([fp/neg, 0])
...
def din_attention(query, facts, attention_size, mask, stag='null', mode='SUM', softmax_stag=1, time_major=False, return_alphas=False, kernel_initializer=None):
    if isinstance(facts, tuple):
        # In case of Bi-RNN, concatenate the forward and the backward RNN outputs.
        facts = tf.concat(facts, 2)
    ...
    d_layer_1_all = keras.layers.Dense(80, activation='sigmoid', kernel_initializer=kernel_initializer, name='f1_att' + stag)
    d_layer_1 = d_layer_1_all(din_all)

    d_layer_2_all = keras.layers.Dense(40, activation='sigmoid', kernel_initializer=kernel_initializer, name='f2_att' + stag)
    d_layer_2 = d_layer_2_all(d_layer_1)

    d_layer_3_all = keras.layers.Dense(1, activation=None, kernel_initializer=kernel_initializer, name='f3_att' + stag)
    d_layer_3 = d_layer_3_all(din_all)

    d_layer_3_all = tf.reshape(d_layer_3, [-1, 1, tf.shape(facts)[1]])
