...
def get_program_arguments():
    ...
    transformer_parser.add_argument("--use-autoregressive-mask-for-test", action="store_true",
                                    help="Run the test with an autoregressive mask in the attention")
...
def dense_transformer_fwd_and_grad(transformer, input_activation):
    ...
    # Multi-head attention
    output_activation = transformer.attention(x, x, x, mask=mask, is_self_attention=True, debug_name="layer_0")
    loss = tf.reduce_sum(output_activation)
...
def main(args):
    ...
    # Configure the IPU
    cfg = ipu.utils.create_ipu_config(profiling=args.profile, profile_execution=args.profile, report_directory="./report/")
