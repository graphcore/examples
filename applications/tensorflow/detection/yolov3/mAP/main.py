...
parser.add_argument('-np', '--plot', help="no plot is shown.", action="store_true")
...
# try to import Matplotlib if the user choose the option --plot
draw_plot = False
if args.plot:
    try:
...
def draw_plot_func(dictionary, n_classes, window_title, plot_title, x_label, output_path, to_show, plot_color, true_p_bar):
    ...
    if true_p_bar != "":
        ...
        plt.barh(range(n_classes), tp_sorted, align='center',
                 color='forestgreen', label='True Predictions', left=fp_sorted)
        # add legend
...
ground_truth_list = read_labels(ground_truth_file)

# dictionary with counter per class
gt_counter_per_class = {}

for file_id, lines_list in enumerate(ground_truth_list):
    bounding_boxes = []
    is_difficult = False
    for line in lines_list:
        try:
            ...
            error_msg = "Error: line in the wrong format.\n"
    ...
    with open(tmp_files_path + "/" + str(file_id) + "_ground_truth.json", 'w') as outfile:
...
predicted_files_list = read_labels(predicted_file)

for class_index, class_name in enumerate(gt_classes):
    bounding_boxes = []
    for file_id, lines in enumerate(predicted_files_list):
        for line in lines:
            try:
            ...
            if tmp_class_name == class_name:
                bbox = left + " " + top + " " + right + " " + bottom
                bounding_boxes.append({"confidence": confidence, "file_id": str(file_id), "bbox": bbox})
    ...
    """
      ...
      """
            if show_animation:
                ...
                output_img_path = results_files_path + "/images/single_predictions/" + \
                    class_name + "_prediction" + str(idx) + ".jpg"
...
for lines_list in predicted_files_list:
    # get lines to list
    for line in lines_list:
