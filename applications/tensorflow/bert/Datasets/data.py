...
def synthetic_pretraining_dataset(opts):
    ...
    if opts['fix_synth_seed'] is True:
        # Set the seed to 1 to ensure test dataset is the same each call
        np.random.seed(1)


    _input_ids = np.random.randint(vocab_size, size=seq_length).astype(np.int32)
    _input_position = np.random.randint(seq_length, size=seq_length).astype(np.int32)
    _token_type_ids = np.random.randint(4, size=seq_length).astype(np.int32)
    _masked_lm_positions = np.random.randint(seq_length, size=max_predictions_per_seq).astype(np.int32)
    _masked_lm_ids = np.random.randint(vocab_size, size=max_predictions_per_seq).astype(np.int32)
    _masked_lm_weights = np.random.randint(2, size=max_predictions_per_seq).astype(np.float32)
    _next_sentence_labels = np.random.randint(1000, size=1).astype(np.int32)
    _mask_padding_index = np.random.randint(20, size=1).astype(np.int32)
    _seq_padding_index = np.random.randint(128, size=1).astype(np.int32)
...
def get_dataset_files_count(opts, is_training=True):
    ...
    for each_input_file in input_files:
        total_samples += sum(1 for _ in tf.python_io.tf_record_iterator(each_input_file))
    return total_samples


def get_pretraining_dataset(opts, is_training=True, num_cpu_threads=4):
    if is_training:
        input_file = opts['train_file']
    else:
        input_file = opts['test_file']
    ...
    if not input_files:
        tf.logging.error("Could no open any of the dataset input files, exiting...")
        sys.exit(1)
    ...
    if is_training:
        ...
        d = d.shuffle(buffer_size=int(opts["pipeline_depth"]*opts["replicas"]))
    else:
...
def get_squad_dataset(opts, is_training):
        ...
        batch_size = opts['batch_size']
        if is_training:
            filename = os.path.join(opts['tfrecord_dir'], "train.tf_record")
        else:
            filename = os.path.join(opts['tfrecord_dir'], "eval.tf_record")
        if not os.path.exists(filename) and not is_training:
            examples = read_squad_examples(input_file=input_file, opts=opts,  is_training=is_training)
...
def data(opts, is_training=True):
        ...
        if opts['task'] == 'pretraining':
            if opts['generated_data']:
                ...
                return get_pretraining_dataset(opts, is_training, num_cpu_threads=opts['parallell_io_threads'])
        elif opts['task'] == 'SQuAD':
            return get_squad_dataset(opts, is_training)
        else:
            raise ValueError("Conflict options between generated-data and input_file")
...
def add_arguments(parser):
    ...
    group.add_argument('--test-file', type=str, required=False,
                       help="path to wiki/corpus test dataset tfrecord file.")
