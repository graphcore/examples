...
from run_squad import build_squad_pipeline_stages, build_network, should_be_pipeline_when_inference, build_infer_network_without_pipeline, build_network
...
def build_graph(opts, iterations_per_step=1, is_training=True):
    ...
    with train_graph.as_default():
        ...
        with ipu_scope('/device:IPU:0'):
            ...
            if embedded and is_training:
                raise ValueError("embedded_runtime is only to be used for inference.")

            train = ipu.ipu_compiler.compile(bert_net, []) if not embedded else None
    ...
    if init_checkpoint_path:
        ...
        (assignment_map, initialized_variable_names) = bert_ipu.get_assignment_map_from_checkpoint(tvars, init_checkpoint_path)
...
def run_time(opts, dataset_list=None):
    ...
    with tf.Session() as sess:
        ...
        def runner(feed_dict, session):
            for step in range(number_of_steps):
                ...
                if step % int(opts['steps_per_logs']) == 0:
                    logger.info(f"{step}/{number_of_steps}\t\t[{100*float(step/number_of_steps):.2f}%]")

                durations.append((start, stop))
                master_durations.append(stop - start)

                if opts['do_predict']:
                    all_results.extend([squad_results.RawResult(this_unique_id, this_start_logit.astype(np.float64), this_end_logit.astype(np.float64))
                                        for this_unique_id, this_start_logit, this_end_logit in zip(ans[0], ans[1], ans[2])])
            thread_queue.put(durations, timeout=10)

        thp = [Thread(target=runner, args=(feed_dict, sess)) for _ in range(opts['num_inference_thread'])]
        ...
        tput = bs * opts['num_inference_thread'] * number_of_steps / (max_stop - min_start)

        test_results = {'batch size': bs, 'latency': latency, 'latency_99': latency_99, 'latency_99_9': latency_99_9, 'throughput': tput}
...
def set_defaults(opts):
    ...
    if opts['generated_data']:
        try:
            ...
            logger.info("Cannot write predictions on synthetic generated data.\n\t\t\t\tExiting.")
            sys.exit(1)
...
def add_squad_options(parser: argparse.ArgumentParser):
    ...
    # Inference
    group.add_argument('--embedded-runtime', action='store_true', default=False,
                       help='Runs model on the embedded runtime.')
    group.add_argument('--dump-raw-timings', action='store_true', default=False,
                       help="Dumps out elapsed times per execution to timings.txt")
    group.add_argument('--num-iter', action='store', default=1000,
                       help="Number of iterations to run inference for.")
    group.add_argument('--num-inference-thread', action='store', default=2,
