...
from common.utils import mask_from_lens_static as mask_from_lens
...
class FFTransformer(nn.Module):
    ...
    def forward(self, dec_inp, seq_lens=None, conditioning=0):
        if self.word_emb is None:
            inp = dec_inp
            mask = mask_from_lens(seq_lens, 870).unsqueeze(2)
        else:
        ...
        mask = mask.float()
        pos_seq = torch.arange(inp.size(1), dtype=inp.dtype)
