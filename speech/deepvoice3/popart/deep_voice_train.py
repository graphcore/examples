...
def create_inputs_for_training(builder, conf):
    ...
    inputs["text_input"] = builder.addInputTensor(
        popart.TensorInfo("INT32", [conf.replica_batch_size, conf.max_text_sequence_length]),
    ...
    inputs["speaker_id"] = builder.addInputTensor(
        popart.TensorInfo("INT32", [conf.replica_batch_size, 1]),
        popart.InputSettings(popart.TileSet.IO, exchange_strategy),
        "speaker_id"
    )
    ...
    inputs["done_labels"] = builder.addInputTensor(
        popart.TensorInfo("INT32", [conf.replica_batch_size, 1, conf.max_spectrogram_length]),
        popart.InputSettings(popart.TileSet.IO, exchange_strategy),
        "done_labels"
    )
