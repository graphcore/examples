...
if __name__ == '__main__':
    ...
    mel_spec_data = np.zeros((conf.device_iterations, conf.global_batch_size,
                              conf.mel_bands, conf.max_spectrogram_length)).astype(np.float32)
    mag_spec_data = np.zeros((conf.device_iterations, conf.global_batch_size,
                              (conf.n_fft//2 + 1), conf.max_spectrogram_length)).astype(np.float32)
    done_data = np.zeros((conf.device_iterations, conf.global_batch_size, 1, conf.max_spectrogram_length)).astype(np.int32)

    speaker_data = np.zeros((conf.device_iterations, conf.global_batch_size)).astype(np.int32)
    text_data = np.zeros((conf.device_iterations, conf.global_batch_size, conf.max_text_sequence_length)).astype(np.int32)
    ...
    for batch_ind in range(conf.device_iterations):
        for sample_ind in range(conf.global_batch_size):
            ...
            speaker_ind = (batch_ind * conf.global_batch_size + sample_ind) % conf.num_speakers
