...
def prepare_triton_environment(request):
    ...
    try:
        ...
        os.system('cp -rfL ' + poplar_sdk_dir_env +
                  '/lib ' + poplar_backend_dir)
        if Path(poplar_sdk_dir_env + '/../libtriton_poplar.so').is_file():
            ...
            os.system('cp -fL /opt/triton/lib/libtriton_poplar.so ' +
                      poplar_backend_dir + '/libtriton_poplar.so')
        else:
            pytest.fail(
                "Can't find library \"libtriton_poplar.so\" needed to run Triton Server.")
...
def triton_server(request, prepare_triton_environment):
    ...
    if not Path(ts_exec_path + "/tritonserver").is_file():
        pytest.fail(str(ts_exec_path) + "/tritonserver doesnt exists!")
    server = TritonServer(request, ts_exec_path, model_repo_opt,
                          grpc_port_opt)
    yield server
    server.terminate()
