...
class TritonServer:
    def __init__(self, request, ts_exe_path, model_repo_opt, backend_dir_opt, grpc_port_opt):
        self.model_repo_path = GetModelPath(request.config, model_repo_opt)

        ts_backend_dir = request.config.getoption(backend_dir_opt)
        ...
        self.ts_proc = subprocess.Popen([
            ts_exe_path, "--model-repository", self.model_repo_path, "--backend-directory",
            ts_backend_dir, grpc_port_opt, ts_gprc_port
        ])
