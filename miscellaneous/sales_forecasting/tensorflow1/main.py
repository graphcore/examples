...
def generic_graph(opts, data, trainFlag):
    ...
    device_iterations = opts.device_iterations if training else opts.validation_device_iterations
    # When replicating, we divide the data stream into N streams, so we only need to do 1/N batches in each stream.
    # For this reason, device_iterations must be a minimum of N.
    device_iterations = int(device_iterations / opts.replication_factor)
    ...
    with graph.as_default():
            ...
            def comp_fn():
                def body(total_loss, total_rmse, batch):
                    ...
                    with tf.control_dependencies([grad_op]):
                        return total_loss + loss, total_rmse + rmse
                return loops.repeat(device_iterations,
                                    body,
        ...
        avg_loss, avg_rmse = [x / device_iterations for x in outputs]
...
def train_process(opts, training_data, valid_data):
    ...
    opts.iterations_per_epoch = training_data._size / (opts.micro_batch_size * opts.device_iterations)
    ...
    if not opts.no_validation:
        opts.validation_device_iterations = valid_data._size // opts.validation_batch_size
...
def preprocess_options(opts):
    ...
    # Ensure there are enough device_iterations to replicate across
    assert opts.device_iterations >= opts.replication_factor, "There must be enough batches (N) to split when replicating across N graphs."
...
def get_options():
    ...
    group.add_argument('--device-iterations',    type=int,   default=8068,
                       help="How many iterations/ global batches to process on the device before returning to the host."
                            "Where a global batch = replication_factor * micro-batch-size. When replication is set to N, "
                            "the data stream is split into N streams, each doing 1/N batches."
