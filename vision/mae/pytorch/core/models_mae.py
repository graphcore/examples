...
class MaskedAutoencoderViT(nn.Module):
        ...
        self.decoder_blocks = nn.ModuleList(
            [
                Block(
                    ...
                    qkv_bias=True,
                    norm_layer=norm_layer,
                    act_layer=gelu) for i in range(decoder_depth)])
    ...
    def _set_pipeline(self):
        ...
        last_id, block_count, stage = self._set_mae(
            self.patch_embed, self.blocks, self.norm, layer_ipu, 0, 0, stage)
        pipeline = self.pipeline[int(num):]
        layer_ipu = self._get_layer_ipu(pipeline)
    ...
    def forward_decoder(self, x, ids_restore, restore_mat):
        ...
        if self.use_half:
            x_ = torch.cat([x[:, 1:, :], mask_tokens.half()],
                           dim=1)  # no cls token
        else:
    ...
    def forward(
        ...
        latent = self.forward_encoder(
            imgs, self.mask_ratio, ids_shuffle, keep_mat)
        pred = self.forward_decoder(
            latent, ids_restore, restore_mat)  # [N, L, p*p*3]
