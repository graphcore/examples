...
const OperatorIdentifier remapCEInplaceId = {"ai.graphcore", "RemapCEInplace",
                                             1};
} // namespace CustomOperators
...
public:
  RemapCEOp(OperatorIdentifier const &opid, Op::Settings const &settings_,
            int64_t fwd_grain_size, int64_t bwd_grain_size,
            bool fwd_clone_layout, bool bwd_clone_layout, bool fwd_after_matmul,
...
public:
  RemapCEInplaceOp(OperatorIdentifier const &opid,
                   ...
                   bool bwd_after_matmul, std::string const &debug_str)
      : RemapCEOp(opid, settings_, fwd_grain_size, bwd_grain_size,
                  fwd_clone_layout, bwd_clone_layout, fwd_after_matmul,
                  bwd_after_matmul, debug_str){};
  RemapCEInplaceOp(const RemapCEOp &op)
      : RemapCEOp(CustomOperators::remapCEInplaceId, op.getSettings(),
                  op.getFwdGrainSize(), op.getBwdGrainSize(),
                  op.isFwdCloneLayout(), op.isBwdCloneLayout(),
                  op.isFwdAfterMatmul(), op.isBwdAfterMatmul(),
                  op.getDebugStr()){};
...
public:
  ...
  poplar::Tensor unwindTensorLayout(poplar::Tensor, InIndex,
                                    OutIndex) const override;
