...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = self.qkv(x).reshape(
            ...
            3,
            self.num_heads,
            torch.div(C, self.num_heads, rounding_mode='trunc')).permute(
            2,
            0,
            3,
            1,
...
class VisionTransformer(nn.Module):
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        if (isinstance(npatch, int) and npatch == N and w == h) or (isinstance(
                npatch, torch.Tensor) and torch.equal(npatch, N) and torch.equal(w, h)):
