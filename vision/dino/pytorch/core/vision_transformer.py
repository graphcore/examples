...
class VisionTransformer(nn.Module):
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        if (isinstance(npatch, int) and npatch == N and w == h) or (
            isinstance(npatch, torch.Tensor) and torch.equal(npatch, N) and torch.equal(w, h)
        ):
    ...
    def prepare_tokens(self, x):
        ...
        if self.precision is Precision.FP32:
            ...
            x = torch.cat((cls_tokens * torch.tensor(1.0, dtype=torch.float16), x), dim=1)
