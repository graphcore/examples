...
import math
from layer.base import DetectBase
...
class AnchorTargetLayer(DetectBase):
    ...
    def forward(self, meta):
        ...
        rpn_label, rpn_keep, rpn_bbox_targets, rpn_bbox_inside_weights, rpn_bbox_outside_weights = self.change2tensor(
            self._forward([
                torch.from_numpy(gt_boxes[:, :num_boxes, :]),
                im_info,
            ]))

        if rpn_keep.numel() == cfg.TRAIN.RPN_BATCHSIZE:
            pass
        else:
            return None

        meta['rpn_label'] = rpn_label.squeeze(0)
        meta['rpn_keep'] = rpn_keep
        meta['rpn_bbox_targets'] = rpn_bbox_targets.squeeze(0)
        meta['rpn_bbox_inside_weights'] = rpn_bbox_inside_weights.squeeze(0)
        meta['rpn_bbox_outside_weights'] = rpn_bbox_outside_weights.squeeze(0)
