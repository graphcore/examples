...
from IPU.basic_func import digit_version
from config import cfg
from IPU.basic_func import deduce_half
...
def roi_align(x,
    ...
    cast_flag, x, fp16_on = deduce_half(x, fp16_on)
    if fp16_on:
        rois = rois.cast('FLOAT16')
    else:
        rois = rois.cast('FLOAT')

    if fp16_on:
        raise NotImplementedError('maybe not implemented')

    if digit_version(popart.__version__) > digit_version('2.6.0+0'):
        rois = rois[0]
        batch_indices = gcop.zeros(rois.pureShape[0], gcop.int32)
        result = gcop.bF.roi_align(x,
                                   rois,
                                   batch_indices,
