...
def roi_align(
    ...
    assert x.dtype in ['FLOAT', 'FLOAT16']
    assert len(rois.pureShape) == 2
    assert rois.dtype in ['FLOAT', 'FLOAT16']
    assert len(batch_indices.pureShape) == 1
    assert batch_indices.dtype in [
        'INT32', 'INT64', 'INT16', 'UINT32', 'UINT64', 'UINT16']
    assert mode in ['avg', 'max']
    args = [x, rois, batch_indices]
    args = [ele.getIpuIndex() for ele in args]
    result = get_builder().aiOnnx.roialign(
        args,
        mode,
        output_height,
        output_width,
...
def recomputation_checkpoint(tensors_or_single_tensor):
    # tensors: list of tensors or single tensor
    if isinstance(tensors_or_single_tensor, TTensor):
        results = get_builder().checkpointOutput([tensors_or_single_tensor.getIpuIndex()])
        assert isinstance(results, list)
        return TTensor(results[0])
    tensors = [t.getIpuIndex() for t in tensors_or_single_tensor]
    results = get_builder().checkpointOutput(tensors)
    results = [TTensor(ele) for ele in results]
    return results
