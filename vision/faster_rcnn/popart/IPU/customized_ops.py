...
from examples_utils import load_lib as load_lib_
...
def load_lib():
    ...
    if not LIB_LOADED:
        ...
        print("Building (if necessary) and loading rnnt_loss custom op.")
        os.environ['NMS_CODELET_PATH'] = os.path.abspath(cd + '/custom_ops/nms/nms_codelet.cpp')
        os.environ['ROI_CODELET_PATH'] = os.path.abspath(cd + '/custom_ops/roi_align/roiAlignCodelets.cpp')
        load_lib_(cd + '/custom_ops/nms/nmsop.cpp')
        load_lib_(cd + '/custom_ops/roi_align/roi_align.cpp')
        LIB_LOADED = True
...
def roi_align(x,
    ...
    batch_size, channels, height, width = x.pureShape
    with bF.name_scope("roiAlign"):
        out = bF.get_builder().customOp(
            opName="roiAlign",
            opVersion=1,
            domain="ai.graphcore",
            inputs=[x.getIpuIndex(),
                    rois.getIpuIndex()],
            attributes={
