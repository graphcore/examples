...
class XMLDataset(CocoDataset):
    def __init__(self, class_names, include_difficult=False, **kwargs):
            ...
            for _object in root.findall("object"):
                ...
                for cat in categories:
                    if category == cat["name"]:
                        cat_id = cat["id"]
                xmin = float(_object.find("bndbox").find("xmin").text) - 1.0
                ymin = float(_object.find("bndbox").find("ymin").text) - 1.0
                xmax = float(_object.find("bndbox").find("xmax").text) - 1.0
                ymax = float(_object.find("bndbox").find("ymax").text) - 1.0
                w = xmax - xmin
                h = ymax - ymin
                if w < 0 or h < 0:
                    logging.warning(
