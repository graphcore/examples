...
class DecodeDetections(Layer):
    ...
    def __init__(self,
        ...
        Arguments:
            ...
            iou_threshold (float, optional): A float in [0,1]. All boxes with a Jaccard similarity of greater than
                `iou_threshold` with a locally maximal box will be removed from the set of predictions for a given
                 class, where 'maximal' refers to the box score.
            ...
            coords (str, optional): The box coordinate format that the model outputs. Must be 'centroids'
                i.e. the format `(cx, cy, w, h)` (box center coordinates, width, and height). Other coordinate formats
                are currently not supported.
            normalize_coords (bool, optional): Set to `True` if the model outputs relative coordinates (i.e.
                coordinates in [0,1]) and you wish to transform these relative coordinates back to absolute
        ...
        self.tf_img_height = tf.constant(self.img_height, dtype=tf.float16, name='img_height')
        self.tf_img_width = tf.constant(self.img_width, dtype=tf.float16, name='img_width')
    ...
    def call(self, y_pred, mask=None):
        ...
        cx = y_pred[..., -12] * y_pred[..., -4] * y_pred[..., -6] + y_pred[..., -8]  # cx = cx_pred * cx_variance *
        # w_anchor + cx_anchor
        cy = y_pred[..., -11] * y_pred[..., -3] * y_pred[..., -5] + y_pred[..., -7]  # cy = cy_pred * cy_variance *
        # h_anchor + cy_anchor
        ...
        def filter_predictions(batch_item):
            ...
            def filter_single_class(index):
                ...
                class_id = tf.fill(dims=tf.shape(confidences), value=tf.cast(index, dtype=tf.float16))
                box_coordinates = batch_item[..., -4:]

                single_class = tf.concat([class_id, confidences, box_coordinates], axis=-1)

                # Apply confidence thresholding with respect to the class defined by `index`.
                threshold_met = single_class[:, 1] > tf.cast(self.tf_confidence_thresh, dtype=tf.float16)
                ...
                def perform_nms():
                    ...
                    # `tf.image.non_max_suppression()` needs the box coordinates in the format `(ymin, xmin, ymax,
                    # xmax)`.
                ...
                def no_confident_predictions():
                    return tf.constant(value=0.0, shape=(1, 6), dtype=tf.float16)
