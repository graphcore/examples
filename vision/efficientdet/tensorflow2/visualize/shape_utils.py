...
def pad_tensor(t, length):
    ...
    pad_shape = tf.cond(
        tf.greater(t_rank, 1), lambda: tf.concat([pad_d0, t_shape[1:]], 0), lambda: tf.expand_dims(length - t_d0, 0)
    )
...
def assert_box_normalized(boxes, maximum_normalized_coordinate=1.1):
    ...
    return tf.Assert(
        tf.logical_and(tf.less_equal(box_maximum, maximum_normalized_coordinate), tf.greater_equal(box_minimum, 0)),
        [boxes],
    )
