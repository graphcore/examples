...
def activation_fn(features: tf.Tensor, act_type: Text):
    ...
    elif act_type == "ipu_swish":
        return ipu.nn_ops.swish(features)
...
def cross_replica_mean(t, num_shards_per_group=None):
    ...
    return tf.tpu.cross_replica_sum(t, group_assignment) / tf.cast(num_shards_per_group, t.dtype)
...
def get_ckpt_var_map(ckpt_path, ckpt_scope, var_scope, skip_mismatch=None):
    ...
    if tf.distribute.get_replica_context():
        replica_id = tf.get_static_value(tf.distribute.get_replica_context().replica_id_in_sync_group)
    else:
        replica_id = 0
    ...
    for i, v in enumerate(model_vars):
        ...
        if not var_op_name.startswith(var_scope):
            logging.info("skip {} -- does not match scope {}".format(var_op_name, var_scope))
        ckpt_var = ckpt_scope + var_op_name[len(var_scope) :]
        if "global_step" in ckpt_var:
            continue

        if ckpt_var not in ckpt_var_names and var_op_name.endswith("/ExponentialMovingAverage"):
        ...
        if v.shape != ckpt_var_name_to_shape[ckpt_var]:
            if skip_mismatch:
                ...
                continue
            raise ValueError(
                "shape mismatch {} ({} vs {})".format(var_op_name, v.shape, ckpt_var_name_to_shape[ckpt_var])
            )
...
class TpuBatchNormalization(tf.keras.layers.BatchNormalization):
    ...
    def _moments(self, inputs, reduction_axes, keep_dims):
        ...
        logging.info("TpuBatchNormalization with num_shards_per_group {}".format(num_shards_per_group))
        if num_shards_per_group > 1:
            # Compute variance using: Var[X]= E[X^2] - E[X]^2.
            shard_square_of_mean = tf.math.square(shard_mean)
            shard_mean_of_square = shard_variance + shard_square_of_mean
            group_mean = cross_replica_mean(shard_mean, num_shards_per_group)
            group_mean_of_square = cross_replica_mean(shard_mean_of_square, num_shards_per_group)
...
class SyncBatchNormalization(tf.keras.layers.BatchNormalization):
    ...
    def _moments(self, inputs, reduction_axes, keep_dims):
        ...
        if num_shards > 1:
            ...
            group_mean_of_square = replica_context.all_reduce(tf.distribute.ReduceOp.MEAN, shard_mean_of_square)
            group_variance = group_mean_of_square - tf.math.square(group_mean)
            return (group_mean, group_variance)
        else:
            return (shard_mean, shard_variance)
...
def parse_image_size(image_size: Union[Text, int, Tuple[int, int]]):
    ...
    raise ValueError("image_size must be an int, WxH string, or (height, width)" "tuple. Was %r" % image_size)
...
def verify_feats_size(feats, feat_sizes, min_level, max_level, data_format="channels_last"):
        ...
        if feats[cnt].shape[h_id] != size["height"]:
            raise ValueError(
                ...
                "(input_height: {}, min_level: {}, max_level: {}.)".format(
                    cnt, feats[cnt].shape, size["height"], feat_sizes[0]["height"], min_level, max_level
                )
            )
        if feats[cnt].shape[w_id] != size["width"]:
            raise ValueError(
                ...
                "(input_width: {}, min_level: {}, max_level: {}.)".format(
                    cnt, feats[cnt].shape, size["width"], feat_sizes[0]["width"], min_level, max_level
                )
            )
...
def set_precision_policy(policy_name: Text = None):
    ...
    assert policy_name in ("float16", "mixed_float16", "mixed_bfloat16", "float32")
...
def build_model_with_precision(pp, mm, ii, *args, **kwargs):
    ...
    NOTE: short argument names are intended to avoid conflicts with kwargs.
    ...
    if pp == "mixed_bfloat16":
        ...
        raise ValueError("Unknown precision name {}".format(pp))
...
def _recompute_grad(f):
    ...
    def inner(*args, **kwargs):
        ...
        def grad_wrapper(*wrapper_args, **grad_kwargs):
            """Wrapper function to accommodate lack of kwargs in graph mode decorator."""
