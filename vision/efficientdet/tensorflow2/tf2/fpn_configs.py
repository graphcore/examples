...
def qufpn_config(min_level, max_level, weight_method=None):
    ...
    i = min_level
    p.nodes.append(
        {"feat_level": i, "inputs_offsets": [node_ids[i][0]] + [level_last_id(i + 1)], "weight_method": p.weight_method}
    )
    node_ids[i].append(next(id_cnt))
    node_ids[max_level].append(node_ids[max_level][-1])
    ...
    for i in range(max_level, min_level - 1, -1):
        # quad-add path.
        p.nodes.append(
            {"feat_level": i, "inputs_offsets": [node_ids[i][2], node_ids[i][4]], "weight_method": p.quad_method}
        )
        node_ids[i].append(next(id_cnt))
