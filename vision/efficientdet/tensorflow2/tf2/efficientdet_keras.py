...
class FNode(tf.keras.layers.Layer):
    ...
    def fuse_features(self, nodes):
        ...
        if self.weight_method == "attn":
            ...
            nodes = [nodes[i] * edge_weights[i] / (weights_sum + 0.0001) for i in range(len(nodes))]
    ...
    def _add_wsm(self, initializer, shape=None):
        for i, _ in enumerate(self.inputs_offsets):
            ...
            self.vars.append(self.add_weight(initializer=initializer, name=name, shape=shape))
...
class OpAfterCombine(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        if self.separable_conv:
            conv2d_layer = functools.partial(tf.keras.layers.SeparableConv2D, depth_multiplier=1)
        else:
        ...
        self.bn = util_keras.build_batch_norm(
            is_training_bn=self.is_training_bn, data_format=self.data_format, strategy=self.strategy, name="bn"
        )
...
class ResampleFeatureMap(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        self.conv2d = tf.keras.layers.Conv2D(
            self.target_num_channels, (1, 1), padding="same", data_format=self.data_format, name="conv2d"
        )
        if model_optimizations:
            for method in model_optimizations.keys():
    ...
    def _upsample2d(self, inputs, target_height, target_width):
        ...
        outputs = tf.cast(
            tf.image.resize(inputs, [target_height, target_width], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR),
            inputs.dtype,
        )
        if self.data_format == "channels_first":
...
class ClassNet(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.classes = self.classes_layer(conv2d_layer, num_classes, num_anchors, name="class-predict")
    ...
    def __init__(
        ...
        **kwargs
        ...
        for i in range(self.repeats):
            ...
            self.boxes = self.boxes_layer(separable_conv, num_anchors, data_format, name="box-predict")
        ...
        for level in range(max_level - min_level):
            self.con2d_ts.append(
                tf.keras.layers.Conv2DTranspose(
                    num_filters, 3, strides=2, padding="same", data_format=data_format, use_bias=False
                )
            )
            self.con2d_t_bns.append(
                util_keras.build_batch_norm(
                    is_training_bn=is_training_bn, data_format=data_format, strategy=strategy, name="bn_" + str(level)
                )
            )
        self.head_transpose = tf.keras.layers.Conv2DTranspose(num_classes, 3, strides=2, padding="same")
    ...
    def __init__(self, config, name="fpn_cells"):
        ...
        if config.fpn_config:
            ...
            self.fpn_config = fpn_configs.get_fpn_config(
                config.fpn_name, config.min_level, config.max_level, config.fpn_weight_method
            )

        self.cells = [FPNCell(self.config, name="cell_%d" % rep) for rep in range(self.config.fpn_cell_repeats)]
...
# MODEL CHANGE: We don't actually call into this. The relevant pre/post processing code has
# been copied out into ipu_automl_io.py, and we instead use the EfficientDetNet directly.
class EfficientDetModel(EfficientDetNet):
    ...
    def _postprocess(self, cls_outputs, box_outputs, scales, mode="global"):
        ...
        if mode == "combined":
            return postprocess.postprocess_combined(self.config.as_dict(), cls_outputs, box_outputs, scales)
        if mode == "tflite":
            if scales is not None:
                # pre_mode should be None for TFLite.
                raise ValueError("scales not supported for TFLite post-processing")
            return postprocess.postprocess_tflite(self.config.as_dict(), cls_outputs, box_outputs)
    ...
    def call(self, inputs, training=False, pre_mode="infer", post_mode="global"):
        ...
        # preprocess.
        inputs, scales = self._preprocessing(inputs, config.image_size, config.mean_rgb, config.stddev_rgb, pre_mode)
        # network.
