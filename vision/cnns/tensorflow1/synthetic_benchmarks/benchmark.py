...
from tensorflow.python.ipu import utils, loops, ipu_infeed_queue, ipu_compiler, config
...
def run(benchmark, opts):
    ...
    with tf.device('cpu'):
        dataset = tf.data.Dataset \
            .range((opts.steps + 2) * opts.device_iterations) \
            .map(lambda i: benchmark.inputs(opts, i)) \
            .repeat() \
            .prefetch(opts.device_iterations)

        if opts.device_iterations > 1 or opts.replicas > 1:
    ...
    with ipu_scope('/device:IPU:0'):
        if opts.device_iterations > 1:
            ...
            out = ipu_compiler.compile(lambda: loops.repeat(opts.device_iterations,
                                                            body,
                                                            [input],
                                                            infeed_queue), [])
        else:
    ...
    with tf.Session() as sess:
        ...
        for i in range(opts.steps):
            ...
            average_batches_per_sec += (opts.device_iterations*opts.replicas/duration)/opts.steps
...
def get_config(opts):
    ...
    if opts.device_id == -1:
        cfg.auto_select_ipus = opts.shards * opts.replicas
    else:
        cfg.select_ipus = [opts.device_id]

    if opts.convolution_options:
        cfg.convolutions.poplar_options = json.loads(opts.convolution_options)

    if opts.matmul_options:
        cfg.matmuls.poplar_options = json.loads(opts.matmul_options)
