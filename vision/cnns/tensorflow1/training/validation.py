...
class LatencyThread:
    ...
    def compute_latency(self):
        ...
        while num_batches < self.total_batches:
            latencies = self.valid.session.run(
                self.valid.ops['latency_per_batch'])
...
def validation_graph_builder(model, data_dict, opts):
    ...
    accuracy = tf.reduce_mean(
        tf.cast(tf.equal(predictions, label), tf.float16))
    return accuracy
...
def validation_graph(model, opts):
    ...
    with valid_graph.as_default():
        ...
        valid_dataset = dataset.data(opts, is_training=False).map(
                ...
                def body(total_accuracy, data_dict):
                    ...
                    if opts['latency']:
                        timestamp_enqueue = timestamp_queue.enqueue(
                            data_dict['timestamp'])
                        return (total_accuracy + (tf.cast(accuracy, tf.float32) / opts["validation_device_iterations"]),
                                timestamp_enqueue)
                    else:
                        return total_accuracy + (tf.cast(accuracy, tf.float32) / opts["validation_device_iterations"])
                accuracy = loops.repeat(int(opts["validation_device_iterations"]),
        ...
        if opts['latency']:
            ...
            timestamp = tf.cast(
                tf.timestamp() - relative_timer.get_start(), tf.float32)
        ...
        if opts['use_popdist']:
            ...
            for var in tf.global_variables():
                broadcast_weights.append(var.assign(
                    hvd.broadcast(var, root_rank=0)))
            global_batch_size_ph = tf.placeholder(dtype=tf.int32, shape=())
            broadcast_global_batch_size = hvd.broadcast(
                global_batch_size_ph, root_rank=0)
    ...
    ipu_options = get_config(ipu_id=opts["select_ipu"],
        ...
        number_of_distributed_batch_norm_replicas=opts.get(
                                 "BN_span", 1),
    ...
    if opts['use_popdist'] and reconfigure:
        ipu_options = popdist.tensorflow.set_ipu_config(
            ipu_options, opts['shards'], configure_device=False)
...
def validation_run(valid, filepath, i, epoch, first_run, opts, latency_thread):
            ...
            with open(csv_path, 'rU') as infile:
                ...
                for row in reader:
                    if row['name'] == name:
                        run = False
                        print(
                            'Skipping validation run on checkpoint: {}'.format(name))
                        break
    else:
        name = None
    ...
    if run:
        ...
        count = int(DATASET_CONSTANTS[opts['dataset']]
...
def validation_only_process(model, opts):
    ...
    if opts["restore_path"] and opts['distributed_worker_index'] == 0:
        if os.path.isdir(opts["restore_path"]):
            ...
            weight_avg_ckpts = [
                c for c in ckpts if not ckpt_pattern_idx.match(c)]
            filenames = training_ckpts + weight_avg_ckpts
            filenames = [f[:-len(".index")] for f in filenames]
        else:
            filenames = sorted([f[:-len(".index")]
                               for f in glob(opts['restore_path'] + '*.index')])
    ...
    for i in range(num_files):
        ...
        epoch = float(global_batch_size * iteration) / \
            DATASET_CONSTANTS[opts['dataset']]['NUM_IMAGES']
        for r in range(opts["repeat"]):
            stats = validation_run(
                valid, None, iteration, epoch, i == 0, opts, latency_thread)
            # Handle skipped case
...
def set_validation_defaults(opts):
    if not opts['validation']:
        ...
        opts['validation_global_batch_size'] = opts['micro_batch_size'] * \
            opts['shards']*opts['replicas']*opts['distributed_worker_count']
        ...
        if opts["device_iterations"] < opts["validation_iterations"]:
            opts["validation_device_iterations"] = int(opts["validation_iterations"] //
                                                       int(round(opts["validation_iterations"] / opts['device_iterations'])))
            opts["validation_iterations"] = int((opts["validation_iterations"] +
                                                 opts["validation_device_iterations"] - 1) / opts["validation_device_iterations"])
        else:
            opts["validation_device_iterations"] = opts["validation_iterations"]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Validation for previously generated checkpoints.', add_help=False)
