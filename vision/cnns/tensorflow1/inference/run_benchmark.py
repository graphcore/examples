...
def run_inference(loop_op: tf.Operation, infeed_queue_initializer: tf.Operation, outfeed_op: tf.Operation,
                  batch_size: int, device_iterations: int, network_name: str,
    ...
    Args:
        ...
        device_iterations: Number of forward passes per step.
    ...
    with tf.Session() as session:
        ...
        for iter_count in range(num_iterations):
            ...
            stop = time.time()
            fps.append(batch_size * device_iterations * num_replicas / (stop - start))
            logging.info(
            ...
            if data == 'real':
                ...
                print('Actual: ',
                      ground_truth[
                          (index + num_replicas * iter_count * device_iterations * batch_size) % len(ground_truth)])
                print('Predicted: ', labels_and_probs)


    if len(fps) < 20:
        print("Warning: Because there are less than 20 iterations, only the last measurement is used to"
...
def main(model_arch: str, images: List, batch_size: int,
         device_iterations: int, loop: bool, num_iterations: int, num_ipus: int, mode: str, data: str,
    ...
    Args:
        ...
        batch_size: Batch size per forward pass.
        device_iterations: Number of iterations to perform on the device per step.
    ...
    run_inference(loop_op, infeed_initializer, outfeed_op, batch_size, device_iterations, config.stem,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--device-iterations', dest='device_iterations', type=int, default=100,
                        help="Number of iterations to run on the device before returning control to the host.")
    ...
    main(args.model_arch, image_filenames, args.batch_size, args.device_iterations, args.loop,
