...
if __name__ == '__main__':
    ...
    # norm layer
    training_command += [
        '--norm-layer', '{' + f'"name": "custom_batch_norm", "momentum": {args.bn_momentum}' + '}'
    ]
    # lr schedule params
    lr_schedule_params = [
    ...
    optimizer_params += [
        f'"weight_decay": {args.lars_weight_decay}'
    ] if args.lars_weight_decay is not None else []
    optimizer_params += [
        f'"eeta": {args.lars_eeta}',
        f'"epsilon": 0'
    ] if args.lars_eeta is not None else []
    training_command += [
