...
def add_poprun_arguments(parser):
    ...
    parser.add_argument('--abs_end_lr', default=None, type=float, required=True)
    ...
    parser.add_argument('--weight_decay', default=None, type=float, required=False)
    parser.add_argument('--optimizer', default=None, type=str, choices=['sgd', 'lars'], required=False)
    parser.add_argument('--micro_batch_size', default=None, type=int, required=True)
    parser.add_argument('--grad_accum', default=None, type=int, required=True)
    parser.add_argument('--num_replicas', default=None, type=int, required=True)
    return parser
...
if __name__ == '__main__':
    ...
    if None in [hosts, vipu_host, vipu_partition]:
        raise ValueError(f'The following environment variables must be defined: '
    ...
    poprun_command = [
        ...
        '--vipu-server-host', vipu_host,
        '--vipu-partition', vipu_partition,
    ...
    training_command = [
        *poprun_command,
        ...
        '--ckpts-per-epoch', 1/4,
        '--first-ckpt-epoch', args.num_epochs % 4,
        '--sweep', 'True'
    ]

    global_batch_size = args.micro_batch_size*args.num_replicas*args.grad_accum
    end_learning_rate_ratio = args.abs_end_lr * (1/(args.init_lr * global_batch_size))

    #global batch size
    training_command += ['--global-batch-size', global_batch_size]
    # label smoothing
    training_command += ['--label-smoothing', args.label_smoothing]
    # l2 regularization
    training_command += ['--l2-regularization', args.weight_decay] if args.weight_decay is not None else []
    # lr schedule params
    lr_schedule_params = [
        f'"initial_learning_rate": {args.init_lr}',
        f'"end_learning_rate_ratio": {end_learning_rate_ratio}',
        f'"epochs_to_total_decay": {args.num_epochs - args.warmup_epochs}',
        f'"power": 2'
    ]
    training_command += [
        '--lr-schedule-params', '{' + ','.join(lr_schedule_params) + '}'
    ]
    # warmup params
    lr_warmup_params = [
        f'"warmup_mode": "shift"',
    ...
    training_command += [
        '--lr-warmup-params', '{' + ','.join(lr_warmup_params) + '}'
    ]
    # optimizer params
    optimizer_params = [f'"momentum": 0.9']
    optimizer_params += [
    ...
    training_command += [
        '--optimizer-params', '{' + ','.join(optimizer_params) + '}'
    ]
