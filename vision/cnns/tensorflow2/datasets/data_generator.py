...
IMAGENET_DS_SIZE = {'train': 1281167, 'test': 50000, 'validation': 50000, 'val': 50000}


def MAP_TF_RECORD_PREFIX(split):
...
class DataGenerator:
    ...
    def get_imagenet(path: str, split: str, seed: Optional[int], cycle_length: int = 4, block_length: int = 4):
        ...
        tfrecord_prefix = MAP_TF_RECORD_PREFIX(split)

        if split == 'train':
            filenames = glob.glob1(path, tfrecord_prefix + '*')
    ...
    def build_imagenet_tf_record(dataset_path: str, split: str, num_threads: int = 32, output_directory: str = '.', imagenet_metadata_file='../imagenet_metadata.txt', bounding_box_file='../imagenet_2012_bounding_boxes.csv'):
        ...
        # The path to the class directories should be constructed from dataset_path
        ...
        # Localization of imagenet_metadata_file and bounding_box_file should be relative to the dataset_path
        tfrecord_prefix = MAP_TF_RECORD_PREFIX(split)

        if tfrecord_prefix == 'train':
            num_shards = 1024
        else:
            num_shards = 128

        if not os.path.exists(dataset_path):
            DataGenerator.logger.error(f'Directory {dataset_path} does not exist')
        ...
        num_directories = len(list(glob.glob(f'{dataset_path}/*/')))

        if num_directories != 1000:
            raise ValueError(
                f'The number of directories in {dataset_path} should be 1000 (=num_classes), but instead it is {num_directories}')
        ...
        build_imagenet_data._process_dataset(tfrecord_prefix, dataset_path, num_shards,
                                             num_threads, synset_to_human, image_to_bboxes, output_directory)

        filenames = glob.glob1(output_directory, tfrecord_prefix + '*')
