...
def vanilla_block(x, filters, kernel_size=3, stride=1, conv_shortcut=False, use_bias=False, name=None):
    ...
    if conv_shortcut:
        shortcut = layers.Conv2D(filters, 1, strides=stride, use_bias=use_bias,
                                 name=f'{name}_0_conv', kernel_initializer=tf.keras.initializers.VarianceScaling())(x)
        shortcut = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name=f'{name}_0_bn')(shortcut)
    else:
        shortcut = x

    x = layers.Conv2D(filters, kernel_size, padding='SAME', strides=stride, use_bias=use_bias,
                      name=f'{name}_1_conv', kernel_initializer=tf.keras.initializers.VarianceScaling())(x)
    ...
    x = layers.Conv2D(filters, kernel_size, padding='SAME', use_bias=use_bias,
                      name=f'{name}_2_conv', kernel_initializer=tf.keras.initializers.VarianceScaling())(x)
...
def bottleneck_block(x, filters, kernel_size=3, stride=1, conv_shortcut=False, use_bias=False, name=None):
    ...
    if conv_shortcut:
        shortcut = layers.Conv2D(4 * filters, 1, strides=stride, use_bias=use_bias,
                                 name=f'{name}_0_conv', kernel_initializer=tf.keras.initializers.VarianceScaling())(x)
        shortcut = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name=f'{name}_0_bn')(shortcut)
    ...
    x = layers.Conv2D(filters, 1, 1, use_bias=use_bias,
                      name=f'{name}_1_conv', kernel_initializer=tf.keras.initializers.VarianceScaling())(x)
    x = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name=f'{name}_1_bn')(x)
    x = layers.Activation('relu', name=f'{name}_1_relu')(x)

    x = layers.Conv2D(filters, kernel_size, stride, padding='SAME', use_bias=use_bias,
                      name=f'{name}_2_conv', kernel_initializer=tf.keras.initializers.VarianceScaling())(x)
    x = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name=f'{name}_2_bn')(x)
    x = layers.Activation('relu', name=f'{name}_2_relu')(x)

    x = layers.Conv2D(4 * filters, 1, 1, use_bias=use_bias,
                      name=f'{name}_3_conv', kernel_initializer=tf.keras.initializers.VarianceScaling())(x)
    x = layers.BatchNormalization(axis=bn_axis, gamma_initializer=bn_gamma_initializer,
                                  epsilon=1.001e-5, name=f'{name}_3_bn')(x)
...
def ResNet18(include_top=True,
    ...
    return resnet.ResNet(stack_fn, False, use_bias, 'resnet18', include_top, weights,
                         input_tensor, input_shape, pooling, classes, layers=layers, **kwargs)
...
def ResNet34(include_top=True,
    ...
    return resnet.ResNet(stack_fn, False, use_bias, 'resnet34', include_top, weights,
                         input_tensor, input_shape, pooling, classes, layers=layers, **kwargs)
...
def ResNet50(include_top=True,
    ...
    return resnet.ResNet(stack_fn, False, use_bias, 'custom_resnet50', include_top, weights,
                         input_tensor, input_shape, pooling, classes, layers=layers, **kwargs)
