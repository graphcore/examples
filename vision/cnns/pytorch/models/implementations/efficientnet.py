...
def create_efficientnet(model_name: str, pretrained: bool = False, num_classes: int =1000, expand_ratio: int=6, group_dim: int=1, inference_mode: bool=False):
    ...
    if pretrained and _creating_modified_en_model(expand_ratio, group_dim):
        pretrained = False
        logging.warn("Pretrained EfficientNet model is not supported if "
...
def _common_efficientnet_manipulation(model: torch.nn.Module, group_dim: int):
    ...
    if group_dim > 1:
        ...
        def group_conv(node):
            ...
            return timm.models.layers.create_conv2d(in_channels=node.in_channels,
